...
export function AirportOverview({ viewMode, onViewModeChange, onOpenModule, activeSection, sectionRequest }: {
  ...
  return (
    <div className="relative h-full min-h-0 overflow-hidden bg-[#04111f]">
      ...
      <div className="pointer-events-none absolute bottom-4 left-4 z-20 rounded-lg border border-white/10 bg-[#06111f]/70 px-3 py-2 text-[8px] text-slate-500 backdrop-blur-lg">HIGH-TECH PARK DIGITAL TWIN · 2D / 3D IOC SPATIAL COMMAND VIEW</div>
    </div>
  );
}
...
function InfoPanelHeader({ mode, expanded, onExpand, onClose }: {
  ...
  return (
    <div className="sticky top-0 z-10 flex items-center justify-between border-b border-white/[.08] bg-[#06111f]/96 px-4 py-3 backdrop-blur-2xl">
      <div><p className="text-[9px] uppercase tracking-[.18em] text-cyan-300">High-Tech Park Digital Twin</p><h2 className="mt-1 text-sm font-semibold text-white">{tr(title)}</h2></div>

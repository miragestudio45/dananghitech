...
const SPATIAL_MODULE_MAP: Record<string, AirportModuleId> = {
  "technology-precinct": "PASSENGERS", semiconductor: "AIRPORT_OPS", "digital-tech": "PASSENGERS",
  biotech: "PASSENGERS", precision: "PASSENGERS", "new-materials": "PASSENGERS",
  "industrial-precinct": "ASSETS_FM", factories: "ASSETS_FM", warehouses: "AIRPORT_OPS",
  labs: "ASSETS_FM", "support-buildings": "ASSETS_FM", "utility-precinct": "AIRPORT_OPS",
  substations: "AIRPORT_OPS", "water-network": "AIRPORT_OPS", wastewater: "SAFETY",
  drainage: "INTELLIGENCE", telecom: "SYSTEMS", "public-lighting": "BMS",
  "land-investment": "PASSENGERS", "available-plots": "PASSENGERS", "negotiation-plots": "PASSENGERS",
  "leased-plots": "PASSENGERS", "development-projects": "PASSENGERS", "transport-logistics": "AIRPORT_OPS",
  "main-gates": "AIRPORT_OPS", "internal-roads": "AIRPORT_OPS", "truck-parking": "AIRPORT_OPS",
  "loading-yards": "AIRPORT_OPS", administration: "SYSTEMS", "utility-plant": "BMS",
  "data-center": "SYSTEMS", "green-corridor": "SAFETY",
};

const HOTSPOT_ALIASES: Record<string, string> = {
  "technology-precinct": "rd-zone", semiconductor: "semiconductor", "digital-tech": "rd-zone",
  biotech: "rd-zone", precision: "rd-zone", "new-materials": "rd-zone",
  "industrial-precinct": "semiconductor", factories: "semiconductor", warehouses: "main-gate",
  labs: "rd-zone", "support-buildings": "ioc", "utility-precinct": "utility",
  substations: "substation", "water-network": "utility", wastewater: "wastewater", drainage: "drainage",
  telecom: "datacenter", "public-lighting": "utility", "land-investment": "available-plots",
  "available-plots": "available-plots", "negotiation-plots": "available-plots", "leased-plots": "rd-zone",
  "development-projects": "rd-zone", "transport-logistics": "main-gate", "main-gates": "main-gate",
  "internal-roads": "main-gate", "truck-parking": "main-gate", "loading-yards": "main-gate",
  administration: "ioc", "utility-plant": "utility", "data-center": "datacenter", "green-corridor": "green-corridor",
};

const READINESS_DOMAINS: Array<[string, string, string, AirportStatus]> = [
  ["Infrastructure & utilities", "96.8%", "Two constraints · no tenant service interruption", "normal"],
  ["Land & tenant services", "94.2%", "Three approvals require coordination", "warning"],
  ["Facility & OT", "97.8%", "Seven alerts · critical services available", "normal"],
  ["IT, data & integration", "99.4%", "Three degraded integrations · failover ready", "normal"],
  ["Safety, security & environment", "92.6%", "Perimeter incident P-07 under response", "critical"],
  ["Traffic & logistics", "95.1%", "Main-gate queue forecast above SLA", "warning"],
];

const READINESS_CHECKS: Array<[string, string, AirportStatus]> = [
  ["Power quality for sensitive tenants", "Feeder F-03 under enhanced monitoring", "warning"],
  ["Emergency response teams", "6/6 teams available", "normal"],
  ["Stormwater and flood readiness", "Pumps staged · N+1 reserve", "warning"],
  ["Wastewater compliance", "Latest sample validated", "normal"],
  ["Mini data center failover", "Last test 2 days ago", "normal"],
  ["Perimeter incident containment", "Response in progress", "critical"],
];

const MATURITY_LEVELS: Array<[string, string, string, string]> = [
  ["L1+", "Digital Spatial Foundation", "Current state: survey, orthophoto, point cloud, DSM/DTM, GIS, BIM and governed CDE", "Current"],
  ["L2", "Connected Monitoring", "Proposed scope: IoT/OT telemetry, EBO/SCADA, alarms, historian and cross-domain operating picture", "Proposed"],
  ["L3", "Simulation & Prediction", "Next phase: flood, pollution dispersion, utility outage, predictive maintenance and energy forecasting", "Next"],
  ["L4", "Integrated Optimization", "Long-term target: coordinated workflows, prescriptive recommendations and human-governed automation", "Target"],
];
...
export function AirportOverview({ viewMode, onViewModeChange, onOpenModule, activeSection, sectionRequest }: {
  ...
  const [activeLayers, setActiveLayers] = useState(() => new Set(["Orthophoto & Survey", "Terrain & Elevation", "Land & Planning", "BIM-GIS & Digital Thread", "Power & Energy", "Water & Drainage", "Environment", "Facilities", "Security & Incidents"]));
  ...
  const visibleHotspots = useMemo(() => AIRPORT_HOTSPOTS.filter((hotspot) => {
    if (hotspot.module === "SPATIAL") return activeLayers.has("Orthophoto & Survey") || activeLayers.has("Point Cloud & Reality Capture") || activeLayers.has("Terrain & Elevation") || activeLayers.has("BIM-GIS & Digital Thread");
    if (hotspot.module === "PASSENGERS") return activeLayers.has("Land & Planning") || activeLayers.has("Tenants & Projects");
    if (hotspot.module === "AIRPORT_OPS") return activeLayers.has("Power & Energy") || activeLayers.has("Water & Drainage") || activeLayers.has("Traffic & Logistics") || activeLayers.has("UAV Airspace");
    if (hotspot.module === "SAFETY") return activeLayers.has("Environment") || activeLayers.has("Security & Incidents");
    if (hotspot.module === "SYSTEMS") return activeLayers.has("Facilities") || activeLayers.has("Underground Utilities");
    return activeLayers.has("Facilities") || activeLayers.has("Power & Energy");
  }), [activeLayers]);
      ...
      <AnimatePresence>
        {layersOpen && (
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className={layersPanelClass}>
            <div className="mb-2 flex items-center justify-between"><div className="flex items-center gap-2 text-[12px] font-semibold text-white"><Layers3 size={14} className="text-cyan-300" />{tr("Data layers")}</div><span className="text-[11px] text-slate-500">{activeLayers.size} {tr("active")}</span></div>
            <div className="grid grid-cols-2 gap-1">{AIRPORT_LAYERS.map((layer) => <button key={layer} onClick={() => setActiveLayers((current) => { const next = new Set(current); next.has(layer) ? next.delete(layer) : next.add(layer); return next; })} className={`rounded-md border px-2 py-1.5 text-left text-[11px] transition ${activeLayers.has(layer) ? "border-cyan-400/25 bg-cyan-400/10 text-cyan-200" : "border-transparent bg-white/[.025] text-slate-500 hover:text-slate-300"}`}>{tr(layer)}</button>)}</div>
          </motion.div>
        )}
      </AnimatePresence>
        ...
        {selected && (
          <motion.aside initial={{ x: 350, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: 350, opacity: 0 }} className="airport-scrollbar absolute bottom-20 right-3 top-14 z-50 w-80 overflow-y-auto rounded-xl border border-white/10 bg-[var(--airport-sidebar)]/93 shadow-2xl backdrop-blur-2xl">
            ...
            <div className="space-y-4 p-4">
              ...
              <div className="grid grid-cols-2 gap-2">{selected.kpis.map((kpi) => <div key={kpi.label} className="rounded-lg border border-white/[.07] bg-white/[.035] p-3"><p className="text-[11px] text-slate-500">{tr(kpi.label)}</p><p className="mt-1 text-lg font-semibold text-white">{kpi.value}</p></div>)}</div>
              <div className="space-y-2 text-[12px] text-slate-400"><div className="flex justify-between"><span>{tr("Last data update")}</span><b className="text-emerald-300">{tr("6 seconds ago")}</b></div><div className="flex justify-between"><span>{tr("Linked systems")}</span><b className="text-white">7</b></div><div className="flex justify-between"><span>{tr("Mapped assets")}</span><b className="text-white">1,284</b></div><div className="flex justify-between"><span>{tr("Open work orders")}</span><b className="text-amber-300">4</b></div></div>
...
function InfoPanelHeader({ mode, expanded, onExpand, onClose }: {
  ...
  const title = mode === "summary" ? "Operational overview"
    : mode === "insights" ? "Operational insights"
      : mode === "hierarchy" ? "High-tech park spatial hierarchy"
...
function OverviewInfoContent({ mode, expanded, flightTrend, spatialSelection, onSpatialSelection, onLocateSpatial, onOpenModule, onViewModeChange }: {
  ...
  if (mode === "summary") {
    return <div className={`grid gap-2 ${expanded ? "grid-cols-3 xl:grid-cols-5" : "grid-cols-2"}`}>{AIRPORT_OVERVIEW_KPIS.map(([label, value, trend], index) => <div key={label} className={`rounded-xl border bg-gradient-to-br p-3 ${index === 8 ? "border-red-400/25 from-red-400/10" : index > 11 ? "border-blue-400/20 from-blue-400/10" : "border-cyan-400/20 from-cyan-400/10"} to-transparent`}><p className="text-[11px] uppercase tracking-[.12em] text-slate-500">{tr(label)}</p><div className="mt-2 flex items-end justify-between gap-2"><strong className="text-xl text-white">{value}</strong><span className="text-[11px] text-cyan-200">{tr(trend)}</span></div></div>)}</div>;
  }

  if (mode === "insights") {
    return <div className={`grid gap-4 ${expanded ? "xl:grid-cols-[1.25fr_.85fr_.85fr]" : "grid-cols-1"}`}><AirportPanel title="Infrastructure demand & capacity" subtitle="Actual and forecast infrastructure demand · 24 hours"><div className="p-3"><AirportTrendChart data={flightTrend} height={expanded ? 300 : 190} color="#38bdf8" /></div></AirportPanel><AirportPanel title="Live operations timeline" subtitle="Cross-domain park events"><div className={`${expanded ? "max-h-[340px]" : "max-h-[230px]"} airport-scrollbar overflow-auto p-3`}><AirportTimeline events={OPERATION_EVENTS} /></div></AirportPanel><AirportPanel title="Incident and readiness feed" subtitle="Prioritized response posture"><div className="space-y-2 p-3">{INCIDENTS.slice(0, expanded ? 8 : 4).map((incident, index) => <div key={incident[0]} className="rounded-lg border border-white/[.06] bg-white/[.025] p-2.5"><div className="flex items-center justify-between"><span className="font-mono text-[11px] text-cyan-300">{incident[0]}</span><AirportStatusBadge status={index === 0 ? "critical" : "warning"} label={incident[2]} /></div><p className="mt-1 text-[12px] font-medium text-white">{tr(incident[1])}</p><p className="mt-1 text-[11px] text-slate-500">{tr(incident[3])} · {incident[4]} · {tr(incident[5])}</p></div>)}</div></AirportPanel></div>;
  }

  if (mode === "maturity") {
    return <div className="space-y-4">
      <div className={`grid gap-3 ${expanded ? "xl:grid-cols-3" : "grid-cols-1"}`}>
        {[["CURRENT STATE", "L1+", "Digital Spatial Foundation", "Survey · Point cloud · DSM/DTM · GIS · BIM · CDE"], ["PROPOSED SCOPE", "L2", "Connected Monitoring", "IoT · SCADA · EBO · Historian · Operational workflows"], ["TARGET EVOLUTION", "L3 → L4", "Simulation to Optimization", "What-if · Prediction · Prescriptive decisions · Governed automation"]].map(([eyebrow, level, title, detail], index) => <div key={eyebrow} className={`rounded-xl border p-4 ${index === 0 ? "border-emerald-400/20 bg-emerald-400/[.05]" : index === 1 ? "border-cyan-400/25 bg-cyan-400/[.07]" : "border-violet-400/20 bg-violet-400/[.05]"}`}><p className="text-[10px] font-bold uppercase tracking-[.16em] text-slate-500">{eyebrow}</p><div className="mt-2 flex items-center justify-between"><b className="text-2xl text-white">{level}</b><AirportStatusBadge status={index === 0 ? "normal" : index === 1 ? "optimized" : "info"} /></div><h3 className="mt-3 text-xs font-semibold text-cyan-200">{title}</h3><p className="mt-2 text-[11px] leading-relaxed text-slate-400">{detail}</p></div>)}
      </div>
      <AirportPanel title="Digital Twin Maturity Level" subtitle="Clear separation between current capability, proposed delivery scope and future target"><div className={`grid gap-3 p-4 ${expanded ? "xl:grid-cols-4" : "grid-cols-1"}`}>{MATURITY_LEVELS.map(([level, name, description, state], index) => <div key={level} className={`rounded-xl border p-4 ${index === 0 ? "border-emerald-400/20 bg-emerald-400/[.05]" : index === 1 ? "border-cyan-400/25 bg-cyan-400/[.07]" : index === 2 ? "border-violet-400/20 bg-violet-400/[.05]" : "border-slate-400/15 bg-white/[.025]"}`}><div className="flex items-center justify-between"><span className="text-lg font-black text-white">{level}</span><AirportStatusBadge status={index === 0 ? "normal" : index === 1 ? "optimized" : index === 2 ? "warning" : "info"} label={state} /></div><h3 className="mt-3 text-xs font-semibold text-cyan-200">{tr(name)}</h3><p className="mt-2 text-[12px] leading-relaxed text-slate-400">{tr(description)}</p></div>)}</div></AirportPanel>
      <div className={`grid gap-4 ${expanded ? "xl:grid-cols-3" : "grid-cols-1"}`}>
        <AirportPanel title="Spatial foundation already available"><div className="grid gap-2 p-4">{["UAV photogrammetry and LiDAR", "Point cloud, orthophoto and terrain", "VN-2000 / Hòn Dấu spatial governance", "BIM-GIS federation and common IDs"].map((item) => <button key={item} onClick={() => onOpenModule("SPATIAL")} className="rounded-lg border border-white/[.06] bg-white/[.025] p-3 text-left hover:border-cyan-400/20"><p className="text-[12px] font-semibold text-white">{tr(item)}</p></button>)}</div></AirportPanel>
        <AirportPanel title="Priority simulation scenarios"><div className="grid gap-2 p-4">{[["Flood & drainage", "Rainfall + terrain + drainage + pump capacity"], ["Pollution dispersion", "Wind-aware chemical release impact zones"], ["Utility outage", "Affected tenants, isolation points and rerouting"], ["Predictive maintenance", "BIM-GIS asset context and remaining useful life"]].map(([name, detail]) => <button key={name} onClick={() => onOpenModule("INTELLIGENCE")} className="rounded-lg border border-white/[.06] bg-white/[.025] p-3 text-left hover:border-violet-400/20"><p className="text-[12px] font-semibold text-white">{tr(name)}</p><p className="mt-1 text-[11px] text-slate-500">{tr(detail)}</p></button>)}</div></AirportPanel>
        <AirportPanel title="Governance guardrails"><div className="space-y-2 p-4 text-[12px] text-slate-400">{["Human approval for high-impact commands", "RBAC, MFA and complete audit trail", "Data quality and model confidence displayed", "Fallback procedures remain available", "Simulation results are decision support, not automatic truth"].map((item) => <div key={item} className="rounded-lg bg-white/[.025] px-3 py-2">✓ {tr(item)}</div>)}</div></AirportPanel>
      </div>
    </div>;
  }

  if (mode === "readiness") {
    return <div className={`grid gap-4 ${expanded ? "xl:grid-cols-[1.2fr_.8fr]" : "grid-cols-1"}`}><AirportPanel title="Park-wide readiness by operating domain" subtitle="Live posture, constraints and contingency readiness"><div className="space-y-2 p-3">{READINESS_DOMAINS.map(([domain, score, detail, status]) => <button key={domain} onClick={() => toast.info(`${tr(domain)} · ${score}`)} className="w-full rounded-lg border border-white/[.06] bg-white/[.025] p-3 text-left"><div className="flex items-center justify-between"><div><p className="text-[12px] font-semibold text-white">{tr(domain)}</p><p className="mt-1 text-[11px] text-slate-500">{tr(detail)}</p></div><div className="text-right"><b className="text-sm text-cyan-200">{score}</b><div className="mt-1"><AirportStatusBadge status={status} /></div></div></div></button>)}</div></AirportPanel><div className="space-y-4"><AirportPanel title="Readiness checklist" subtitle="Critical pre-operation checks"><div className="space-y-2 p-3">{READINESS_CHECKS.map(([label, result, status]) => <div key={label} className="flex items-center justify-between gap-3 rounded-lg bg-white/[.025] p-3"><div><p className="text-[12px] text-slate-200">{tr(label)}</p><p className="mt-1 text-[11px] text-slate-500">{tr(result)}</p></div><AirportStatusBadge status={status} /></div>)}</div></AirportPanel><AirportPanel title="Next operational decision"><div className="p-4"><div className="rounded-xl border border-amber-400/20 bg-amber-400/[.06] p-4"><p className="text-[12px] font-semibold text-amber-200">{tr("Main-gate logistics queue and feeder F-03 risk forecast")}</p><p className="mt-2 text-[11px] leading-relaxed text-slate-400">{tr("Rebalance truck appointments, hold reserve power capacity and keep the incident team on enhanced monitoring.")}</p><div className="mt-3 flex gap-2"><button onClick={() => toast.success(tr("Readiness action acknowledged"))} className="airport-button !border-emerald-400/20 !bg-emerald-400/10 !text-emerald-200">{tr("Acknowledge")}</button><button onClick={() => onOpenModule("PASSENGERS")} className="airport-button">{tr("Open land & tenant services")}</button></div></div></div></AirportPanel></div></div>;
  }
  ...
  return <div className={`grid gap-4 ${expanded ? "xl:grid-cols-[1.2fr_.8fr]" : "grid-cols-1"}`}><div className={`grid gap-3 ${expanded ? "grid-cols-2 xl:grid-cols-3" : "grid-cols-1"}`}>{AIRPORT_SPATIAL_HIERARCHY.map((parent) => <section key={parent.id} className={`rounded-xl border p-3 transition ${spatialSelection?.parent.id === parent.id ? "border-cyan-400/25 bg-cyan-400/[.06]" : "border-white/[.07] bg-white/[.025]"}`}><button onClick={() => onSpatialSelection({ parent, node: parent })} className="flex w-full items-center justify-between text-left"><div><h3 className="text-xs font-semibold text-white">{tr(parent.label)}</h3><p className="mt-0.5 text-[10px] uppercase tracking-wider text-cyan-300">{tr(parent.type)}</p></div><MapPinned size={14} className="text-slate-500" /></button><div className="mt-2 space-y-1">{parent.children?.map((child) => <button key={child.id} onClick={() => onSpatialSelection({ parent, node: child })} className={`flex w-full items-center justify-between rounded px-2 py-1.5 text-[12px] transition ${spatialSelection?.node.id === child.id ? "bg-cyan-400/10 text-cyan-200" : "text-slate-400 hover:bg-cyan-400/[.07] hover:text-cyan-200"}`}><span>{tr(child.label)}</span><ChevronRight size={11} /></button>) ?? <button onClick={() => onSpatialSelection({ parent, node: parent })} className="flex w-full items-center justify-between rounded px-2 py-1.5 text-[12px] text-slate-500 hover:bg-cyan-400/[.07] hover:text-cyan-200"><span>{tr("Open facility context")}</span><ChevronRight size={11} /></button>}</div></section>)}</div><AirportPanel title={spatialSelection ? spatialSelection.node.label : "Spatial context detail"} subtitle={spatialSelection ? `${spatialSelection.parent.label} · ${spatialSelection.node.type}` : "Select any park precinct, plot, facility or network to open its operational context"}><div className="p-4">{spatialSelection ? <div className="space-y-4"><div className="flex items-center justify-between"><AirportStatusBadge status={alerts > 5 ? "warning" : "normal"} /><span className="text-[11px] text-slate-500">{tr("Data refreshed 6 seconds ago")}</span></div><div className="grid grid-cols-2 gap-2">{[["Mapped assets", assets.toLocaleString()], ["Linked systems", systems], ["Open alerts", alerts], ["Operational readiness", `${readiness}%`]].map(([label, value]) => <div key={String(label)} className="rounded-lg border border-white/[.06] bg-white/[.025] p-3"><p className="text-[11px] text-slate-500">{tr(String(label))}</p><p className="mt-1 text-lg font-semibold text-white">{value}</p></div>)}</div><div className="space-y-2 rounded-xl border border-white/[.06] bg-white/[.02] p-3 text-[12px]"><div className="flex justify-between"><span className="text-slate-500">{tr("Spatial path")}</span><b className="text-right text-cyan-200">{tr(spatialSelection.parent.label)} / {tr(spatialSelection.node.label)}</b></div><div className="flex justify-between"><span className="text-slate-500">{tr("Primary module")}</span><b className="text-white">{tr(selectedModule)}</b></div><div className="flex justify-between"><span className="text-slate-500">{tr("Data quality")}</span><b className="text-emerald-300">98.4%</b></div></div><div className="grid gap-2"><button onClick={() => onLocateSpatial(spatialSelection)} className="airport-button w-full justify-center"><LocateFixed size={14} />{tr("Locate on park overview")}</button>{selectedModule !== "OVERVIEW" && <button onClick={() => onOpenModule(selectedModule)} className="airport-button w-full justify-center"><Route size={14} />{tr("Open related module")}</button>}<button onClick={() => onViewModeChange("3d")} className="airport-button w-full justify-center"><Box size={14} />{tr("Open in 3D")}</button></div></div> : <div className="grid min-h-64 place-items-center text-center"><div><MapPinned className="mx-auto text-cyan-300" /><p className="mt-3 text-xs font-semibold text-white">{tr("Select a park area")}</p><p className="mt-1 text-[12px] leading-relaxed text-slate-500">{tr("The selected area will reveal its assets, systems, alerts, readiness and linked operational module.")}</p></div></div>}</div></AirportPanel></div>;
}

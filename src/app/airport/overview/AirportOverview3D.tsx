...
export function AirportOverview3D({ onBack2D }: { onBack2D: () => void }) {
  ...
  return (
    <div className="relative h-full min-h-0 overflow-hidden" style={{ backgroundColor: "var(--airport-bg)" }}>
      ...
      <div className="pointer-events-none absolute inset-x-0 top-0 h-40 bg-gradient-to-b to-transparent" style={{ backgroundImage: "linear-gradient(to bottom, color-mix(in srgb, var(--airport-bg) 50%, transparent), transparent)" }} />

      <div className="absolute left-3 top-3 z-40 flex h-9 items-center gap-2 rounded-xl border border-cyan-400/25 bg-[var(--airport-sidebar)]/88 px-3 text-[12px] font-semibold text-cyan-100 shadow-xl backdrop-blur-xl">
      ...
      <div className="absolute left-3 top-14 z-40 flex flex-col gap-2">
        {sceneMode === "factory" ? (
          <button onClick={backToOverview} disabled={Boolean(manualTransition)} className="airport-button bg-[var(--airport-sidebar)]/88 backdrop-blur-xl disabled:cursor-wait disabled:opacity-60">
            <ArrowLeft size={14} /> Back to Overview 3D
          </button>
        ) : (
          <button onClick={onBack2D} className="airport-button bg-[var(--airport-sidebar)]/82 backdrop-blur-xl"><Undo2 size={14} /> {tr("Back to 2D")}</button>
        )}
        <div className="flex overflow-hidden rounded-xl border border-white/10 bg-[var(--airport-sidebar)]/82 p-1 shadow-xl backdrop-blur-xl">
          <button
        ...
        <button onClick={resetCamera} className="airport-button bg-[var(--airport-sidebar)]/82 backdrop-blur-xl"><RotateCcw size={14} /> {tr("Reset camera")}</button>
        <div className="flex gap-1.5">
          <button
            onClick={() => setSettingsPanel((current) => current === "camera" ? null : "camera")}
            className={`airport-button flex-1 justify-center bg-[var(--airport-sidebar)]/82 px-2 backdrop-blur-xl ${settingsPanel === "camera" ? "border-cyan-400/35 text-cyan-200" : ""}`}
          ...
          <button
            ...
            className={`airport-button flex-1 justify-center bg-[var(--airport-sidebar)]/82 px-2 backdrop-blur-xl ${settingsPanel === "lighting" ? "border-cyan-400/35 text-cyan-200" : ""}`}
      ...
      {settingsPanel && (
        <div className="absolute left-[132px] top-[116px] z-50 w-[252px] rounded-2xl border border-cyan-400/20 bg-[var(--airport-panel)]/94 p-4 text-white shadow-2xl backdrop-blur-2xl">
      ...
      {!AIRPORT_3D_CONFIG.modelUrl && (
        <div className="pointer-events-none absolute left-1/2 top-[16%] w-[520px] -translate-x-1/2 rounded-2xl border border-cyan-400/15 bg-[var(--airport-panel)]/72 p-5 text-center backdrop-blur-xl">
      ...
      <div className="absolute right-4 top-[78px] z-40 rounded-2xl border border-cyan-400/18 bg-[var(--airport-panel)]/88 p-3 text-white shadow-2xl backdrop-blur-xl">
        <p className="text-[11px] uppercase tracking-[.18em] text-cyan-300">Compass</p>
        <div className="mt-2 flex items-center gap-3">
          <div className="relative grid h-16 w-16 place-items-center rounded-full border border-white/10" style={{ backgroundColor: "var(--airport-panel-strong)" }}>
      ...
      {sceneMode === "overview" && ready && !transitioning && (
        <div className="pointer-events-none absolute bottom-24 right-5 z-30 max-w-[310px] rounded-xl border border-cyan-400/25 bg-[var(--airport-panel)]/88 p-4 text-white shadow-2xl backdrop-blur-xl">
      ...
      {controlMode === "walk" && (
        <div className="absolute bottom-24 left-5 z-40 w-[340px] rounded-xl border border-cyan-400/20 bg-[var(--airport-panel)]/86 p-4 text-xs text-white shadow-2xl backdrop-blur-xl">
      ...
      {transitionVisible && (
        <div
          className="pointer-events-none absolute inset-0 z-30 grid place-items-center transition-colors duration-500"
          style={{ backgroundColor: manualTransition === "exiting" ? "var(--airport-bg)" : "color-mix(in srgb, var(--airport-bg) 55%, transparent)" }}
        >
          <div className="w-[min(88vw,360px)] rounded-2xl border border-cyan-300/25 bg-[var(--airport-sidebar)]/88 p-5 text-center text-white shadow-[0_24px_80px_var(--airport-shadow-heavy)] backdrop-blur-2xl">
      ...
      {loading && !transitionVisible && <div className="absolute bottom-24 left-1/2 w-72 -translate-x-1/2 rounded-xl border border-white/10 bg-[var(--airport-panel)]/92 p-3 shadow-2xl backdrop-blur-xl"><div className="flex justify-between text-[12px] text-slate-300"><span>{tr("Loading high-tech park reference model")}</span><b>{progress}%</b></div><div className="mt-2 h-1 overflow-hidden rounded bg-white/10"><div className="h-full bg-cyan-300 transition-[width]" style={{ width: `${progress}%` }} /></div></div>}
      ...
      {selectedTarget && <div className="absolute bottom-24 right-5 rounded-xl border border-cyan-400/20 bg-[var(--airport-panel)]/92 p-4 text-xs text-white shadow-2xl backdrop-blur-xl"><p className="text-[11px] uppercase tracking-wider text-cyan-300">{tr("Selected 3D context")}</p><b className="mt-1 block text-sm text-white">{tr(targetLabel ?? "")}</b><p className="mt-1 text-[11px] text-slate-500">{tr("Click Reset camera to return to the high-tech park overview.")}</p></div>}

...
export function AirportHeader({ viewMode, onViewModeChange, spatialContext, onSpatialContextChange }: {
  ...
  return (
    <header className="relative z-[85] flex h-[72px] flex-none items-center gap-2 overflow-hidden border-b border-white/[0.08] bg-[#04101e]/94 px-3 shadow-[0_8px_35px_rgba(0,0,0,.22)] backdrop-blur-2xl xl:px-4">
      ...
      <div className="flex min-w-0 flex-1 items-center justify-center gap-2 px-1 2xl:px-3">
        <div className="flex rounded-lg border border-white/[0.08] bg-white/[0.035] p-0.5">
          {(["2d", "3d"] as const).map((mode) => (
            <button key={mode} onClick={() => onViewModeChange(mode)} className={`rounded-md px-3 py-1.5 text-[11px] font-bold uppercase transition ${viewMode === mode ? "bg-cyan-300 text-slate-950" : "text-slate-500 hover:text-white"}`}>{mode}</button>
          ))}
        </div>

  ...
  return <AnimatePresence>
    {open && <>
      <motion.button
        ...
        onClick={onClose}
        className="fixed bottom-0 left-0 right-0 top-[72px] z-[120] cursor-default bg-black/70 backdrop-blur-sm"
      />
      <motion.div
      ...
      >
        <div className="flex items-center gap-3 border-b border-white/[.08] px-4 py-3">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-[11px] font-semibold uppercase tracking-[.18em] text-cyan-300">{vi ? "Vị trí thiết bị" : "Asset location"}</p>
        ...
        <div className="grid min-h-0 flex-1 grid-cols-1 xl:grid-cols-[minmax(0,1fr)_320px]">
          ...
          <aside className="airport-scrollbar min-h-0 overflow-y-auto border-l border-white/[.08] bg-[#08172a]/88 p-4">
            ...
            <div className="mt-3 space-y-2">
              ...
              ].map(([label,value]) => <div key={label} className="flex items-start justify-between gap-3 rounded-lg border border-white/[.07] bg-white/[.03] px-3 py-2.5"><span className="text-[12px] text-slate-500">{label}</span><b className="text-right text-[12px] text-cyan-200">{value}</b></div>)}
            </div>
            <p className="mt-5 text-[11px] font-semibold uppercase tracking-[.16em] text-slate-500">{vi ? "Thông số vận hành" : "Operating values"}</p>
            <div className="mt-3 space-y-2">{equipment.parameters.map(parameter => <div key={parameter.key} className="rounded-lg border border-white/[.07] bg-white/[.03] px-3 py-2.5"><div className="flex items-center justify-between gap-3"><span className="text-[12px] text-slate-400">{parameter.label}</span><b className="text-[12px] text-white">{String(parameter.value)} {parameter.unit}</b></div></div>)}</div>
          </aside>
        </div>
      </motion.div>
    </>}

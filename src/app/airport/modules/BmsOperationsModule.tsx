...
const BMS_ALARMS = [
  ...
  { id: "BMS-ALM-2403", time: "09:58:40", priority: "Info", asset: "DHTP-CRAH-DC-03", message: "Redundancy test completed", state: "Closed", owner: "Data Center" },
];
      ...
      <AirportPanel title={vi ? "Kiến trúc vận hành EBO / BMS" : "EBO / BMS operational architecture"} subtitle={vi ? "Từ thiết bị hiện trường đến giám sát, điều khiển và Digital Twin" : "From field devices to monitoring, control and Digital Twin"}>
        <div className="grid grid-cols-5 gap-2 p-4">
          {[
            ...
            <p className="text-[11px] font-semibold tracking-[.15em] text-cyan-300">{step}</p>
      ...
      <AirportPanel title={vi ? "Trạng thái hệ thống" : "System status"}>
        <div className="space-y-2 p-3">
          {[
            ...
            <span className="text-[12px] text-slate-300">{name}</span><div className="flex items-center gap-2"><b className="text-[12px] text-white">{value}</b><AirportStatusBadge status={status as "normal" | "warning"} /></div>
          </div>)}
        </div>
      </AirportPanel>
    </div>
    ...
    <AirportPanel title={vi ? "Thiết bị HVAC trọng yếu" : "Critical HVAC equipment"} subtitle={vi ? "Mở nhanh giám sát, mô hình 3D, schematic và điều khiển" : "Open monitoring, 3D, schematic and control"}>
      <div className="grid grid-cols-2 gap-3 p-4 xl:grid-cols-3">
        {BMS_EQUIPMENT_LIST.map((equipment, index) => <button key={equipment.id} onClick={() => onOpenEquipment(equipment.id)} className={`rounded-xl border p-4 text-left transition ${selected.id === equipment.id ? "border-cyan-400/30 bg-cyan-400/[.08]" : "border-white/[.07] bg-white/[.025] hover:border-cyan-400/20 hover:bg-cyan-400/[.04]"}`}>
          ...
          <p className="mt-0.5 text-[12px] text-slate-500">{vi ? equipment.nameVi : equipment.category}</p>
...
function EboCommandCenter({ selected, onSelect }: { selected: BMSEquipment; onSelect: (id: string) => void }) {
  ...
  return <>
    <div className="grid grid-cols-3 gap-2 xl:grid-cols-6">
      {[[vi ? "Máy chủ EBO" : "EBO servers", "3 / 3", "Online"],[vi ? "Automation Server" : "Automation servers", "24 / 24", "Healthy"],[vi ? "Điểm đang quét" : "Scanning points", "68,420", "6 sec"],[vi ? "Lịch đang chạy" : "Active schedules", "184", "Park-wide"],[vi ? "Ghi đè điều khiển" : "Control overrides", "7", "Authorized"],[vi ? "Cảnh báo ưu tiên" : "Priority alarms", "3", "Action"]].map(([a,b,c],i)=><AirportMetricCard key={a} label={a} value={b} trend={c} compact tone={i===5?"amber":"cyan"}/>)}</div>
    <div className="grid gap-4 xl:grid-cols-[260px_1fr]">
      <AirportPanel title={vi ? "Cây hệ thống EBO" : "EBO system tree"}>
        <div className="space-y-1 p-3">
          {[{group:vi?"Trung tâm R&D và Đổi mới sáng tạo":"R&D and Innovation Center", ids:["ahu","fcu","vav"]},{group:vi?"Nhà máy tiện ích trung tâm":"Central Utility Plant",ids:["chiller","plate-heat-exchanger"]},{group:vi?"Trung tâm dữ liệu":"High-Tech Park Mini Data Center",ids:["crah-unit"]}].map(group=><div key={group.group} className="rounded-lg border border-white/[.055] bg-white/[.02] p-2"><p className="mb-1 px-2 py-1 text-[11px] font-semibold uppercase tracking-[.12em] text-slate-500">{group.group}</p>{group.ids.map(id=>{const eq=BMS_EQUIPMENT_LIST.find(e=>e.id===id)!; return <button key={id} onClick={()=>onSelect(id)} className={`flex w-full items-center gap-2 rounded px-2 py-2 text-left text-[12px] ${selected.id===id?"bg-cyan-400/10 text-cyan-200":"text-slate-400 hover:bg-white/[.04] hover:text-white"}`}><span className="h-1.5 w-1.5 rounded-full bg-emerald-300"/>{eq.name}<span className="ml-auto font-mono text-[10px] text-slate-600">{getAssetLocationInfo(id)?.code}</span></button>})}</div>)}
        </div>
      </AirportPanel>
...
function EquipmentLibrary({ selected, onSelect }: { selected: BMSEquipment; onSelect: (id: string) => void }) {
  ...
  return <div className="flex h-full min-h-0 flex-col gap-3">
    ...
    <div className="grid min-h-0 flex-1 items-stretch gap-3 xl:grid-cols-[300px_minmax(0,1fr)] 2xl:grid-cols-[330px_minmax(0,1fr)]">
      <AirportPanel title={vi ? "Thư viện thiết bị BMS" : "BMS equipment library"} subtitle={vi ? "Các model hiện tại được giữ tạm để thay bằng asset Khu Công nghệ cao" : "Current models are retained temporarily for replacement with high-tech park assets"} className="h-full">
        <div className="airport-scrollbar grid h-full grid-cols-2 content-start gap-2 overflow-y-auto p-3">
          {BMS_EQUIPMENT_LIST.map((equipment) => <div key={equipment.id} className={`overflow-hidden rounded-xl border transition ${selected.id === equipment.id ? "border-cyan-400/30 bg-cyan-400/[.09]" : "border-white/[.06] bg-white/[.025] hover:bg-white/[.045]"}`}>
            <button onClick={() => onSelect(equipment.id)} className="w-full p-3 text-left">
              <EquipmentIcon equipment={equipment} /><p className="mt-3 text-[13px] font-semibold text-white">{equipment.name}</p><p className="mt-1 text-[11px] text-slate-500">{vi ? equipment.nameVi : equipment.category}</p>
            </button>
            <div className="border-t border-white/[.06] p-2">
              <button onClick={() => { onSelect(equipment.id); setLocationEquipment(equipment); }} className="flex w-full items-center justify-center gap-1.5 rounded-lg border border-cyan-400/15 bg-cyan-400/[.055] px-2 py-2 text-[11px] font-semibold text-cyan-200 transition hover:bg-cyan-400/[.1]"><MapPin size={11}/>{vi ? "Vị trí thiết bị" : "Asset location"}</button>
            </div>
          </div>)}
        </div>
      </AirportPanel>
  ...
  return <>
    ...
    <div className="grid gap-4 xl:grid-cols-[.75fr_1.25fr]">
      <AirportPanel title={vi?"Bản đồ zone tiện nghi công trình":"Facility comfort zones"}>
        <div className="grid grid-cols-4 gap-2 p-4">{Array.from({length:24},(_,i)=>{const warn=[6,11,18,21].includes(i); return <button key={i} onClick={()=>toast.info(`T1 Zone ${String.fromCharCode(65+(i%6))}-${Math.floor(i/6)+1}`)} className={`aspect-square rounded-lg border p-2 text-left ${warn?"border-amber-400/25 bg-amber-400/[.08]":"border-cyan-400/15 bg-cyan-400/[.045]"}`}><span className="text-[10px] text-slate-500">ZONE</span><b className="block text-[12px] text-white">{String.fromCharCode(65+(i%6))}-{Math.floor(i/6)+1}</b><span className={`mt-2 block text-[11px] ${warn?"text-amber-200":"text-emerald-300"}`}>{warn?"25.8°C":"23.2°C"}</span></button>})}</div>
      </AirportPanel>
  ...
  return <>
    <AirportPanel className={`relative flex h-full min-h-0 flex-col overflow-hidden ${compact ? "" : "min-h-[680px]"}`} title={`${equipment.name} · ${vi ? equipment.nameVi : equipment.category}`} subtitle={info ? `${info.code} · ${info.building} · ${info.area}` : equipment.category} action={<AirportStatusBadge status={enabled ? "normal" : "offline"} label={enabled ? (vi?"Đang chạy":"Running") : (vi?"Đã dừng":"Stopped")} />}>
      <div className="flex flex-wrap items-center gap-1 border-b border-white/[.07] px-3 py-2">
        {views.map(([id,label])=><button key={id} onClick={()=>setView(id)} className={`rounded-lg px-3 py-2 text-[12px] font-semibold transition ${view===id?"bg-cyan-400/15 text-cyan-200":"text-slate-500 hover:bg-white/[.04] hover:text-white"}`}>{label}</button>)}
...
function AssetInfoPanel({ equipment, info, onClose, onLocate }: { equipment: BMSEquipment; info?: ReturnType<typeof getAssetLocationInfo>; onClose: () => void; onLocate: () => void }) {
  const { language } = useAirportLanguage(); const vi = language === "vi";
  return <aside className="airport-scrollbar w-[300px] flex-none overflow-y-auto border-l border-white/[.08] bg-[var(--airport-panel-alt)]/88">
    <div className="sticky top-0 z-10 flex items-center justify-between border-b border-white/[.08] bg-[var(--airport-panel-alt)]/96 px-4 py-3"><div><p className="text-[11px] font-semibold uppercase tracking-[.16em] text-cyan-300">{vi?"Thông tin asset":"Asset information"}</p><p className="mt-0.5 text-[12px] text-slate-500">{info?.code ?? equipment.name}</p></div><button onClick={onClose} className="airport-icon-button !h-8 !w-8"><X size={14}/></button></div>
    <div className="space-y-2 p-3">{[
    ...
    ].map(([label,value])=><div key={label} className="rounded-lg border border-white/[.07] bg-white/[.03] p-3"><p className="text-[11px] text-slate-500">{label}</p><p className="mt-1 text-[12px] font-semibold text-cyan-200">{value}</p></div>)}
    ...
    <p className="pt-3 text-[11px] font-semibold uppercase tracking-[.14em] text-slate-600">{vi?"Thông số vận hành":"Operating values"}</p>
    {equipment.parameters.map(parameter=><div key={parameter.key} className="flex items-center justify-between gap-2 rounded-lg border border-white/[.06] bg-white/[.025] px-3 py-2.5"><div className="flex items-center gap-2"><span className={`h-1.5 w-1.5 rounded-full ${parameter.status==="warning"?"bg-amber-300":"bg-emerald-300"}`}/><span className="text-[11px] text-slate-400">{parameter.label}</span></div><b className={`text-[12px] ${parameter.status==="warning"?"text-amber-200":"text-emerald-300"}`}>{String(parameter.value)} <span className="text-[10px] font-normal text-slate-600">{parameter.unit}</span></b></div>)}
    </div>
  </aside>;
}
...
function MonitorView({ equipment, trend }: { equipment: BMSEquipment; trend: ReturnType<typeof makeTrend> }) {
  ...
  return <div className="grid gap-4 p-4 xl:grid-cols-[.9fr_1.1fr]">
    <div className="grid grid-cols-2 gap-2 self-start">{equipment.parameters.map(parameter=><div key={parameter.key} className="rounded-xl border border-white/[.07] bg-white/[.03] p-3"><div className="flex items-start justify-between gap-2"><p className="text-[11px] uppercase tracking-[.1em] text-slate-500">{parameter.label}</p><AirportStatusBadge status={statusFromParameter(parameter)} /></div><p className="mt-3 text-lg font-semibold text-white">{String(parameter.value)} <span className="text-[12px] font-normal text-slate-500">{parameter.unit}</span></p></div>)}</div>
    <div className="space-y-4"><AirportPanel title={vi?"Xu hướng vận hành 24 giờ":"24-hour operational trend"}><div className="p-3"><AirportTrendChart data={trend} height={260} color={equipment.accentColor}/></div></AirportPanel><div className="grid grid-cols-3 gap-2">{[[vi?"Runtime":"Runtime","4,286 h"],[vi?"Cảnh báo":"Alarms","1 active"],[vi?"Lệnh gần nhất":"Last command","10:38:12"]].map(([a,b],i)=><div key={a} className="rounded-lg border border-white/[.06] bg-white/[.025] p-3"><p className="text-[11px] text-slate-500">{a}</p><p className={`mt-1 text-[13px] font-semibold ${i===1?"text-amber-200":"text-white"}`}>{b}</p></div>)}</div></div>
  </div>;
}
...
function Equipment2D({ equipment }: { equipment: BMSEquipment }) {
  ...
  return <div className="p-4"><div className="relative min-h-[470px] overflow-hidden rounded-xl border border-cyan-400/15 bg-[var(--airport-panel)] p-6"><div className="flex h-full min-h-[390px] items-center justify-center"><div className="flex min-w-[900px] items-center justify-center gap-2">{nodes.map((node,index)=><React.Fragment key={node}><div className={`min-w-[130px] rounded-xl border p-4 text-center ${node===equipment.name?"border-cyan-300/45 bg-cyan-400/[.12]":"border-white/[.09] bg-[var(--airport-panel-alt)]"}`}><div className="mx-auto grid h-9 w-9 place-items-center rounded-lg bg-cyan-400/10 text-cyan-300">{index%2?<Gauge size={18}/>:<Fan size={18}/>}</div><p className="mt-3 text-[12px] font-semibold text-white">{node}</p><p className="mt-1 text-[11px] text-emerald-300">{index===0?"Input":"Live"}</p></div>{index<nodes.length-1&&<div className="relative h-[2px] w-14 bg-cyan-400/35"><span className="absolute right-0 top-1/2 -translate-y-1/2 text-cyan-300">▶</span></div>}</React.Fragment>)}</div></div><div className="absolute bottom-4 left-4 rounded-lg border border-white/[.08] bg-[var(--airport-panel)]/90 px-3 py-2 text-[11px] text-slate-400">{info?.code} · {info?.building} · {info?.floor}</div></div></div>;
}
...
function EquipmentSchematic({ equipment }: { equipment: BMSEquipment }) {
  ...
  return <div className="grid gap-4 p-4 xl:grid-cols-[1.25fr_.75fr]"><div className="relative min-h-[470px] overflow-hidden rounded-xl border border-cyan-400/15 bg-[var(--airport-panel)] p-5"><div className="absolute inset-0 opacity-40" style={{backgroundImage:"radial-gradient(circle at 1px 1px,rgba(34,211,238,.18) 1px,transparent 0)",backgroundSize:"22px 22px"}}/><div className="relative flex h-full min-h-[420px] flex-col justify-between"><div className="flex items-center justify-between"><SchematicNode label={equipment.name} value="RUN" primary/><SchematicLine/><SchematicNode label={vi?"Bộ điều khiển EBO":"EBO controller"} value="AUTO"/><SchematicLine/><SchematicNode label={vi?"Zone / phụ tải":"Zone / load"} value="ACTIVE"/></div><div className="mx-auto grid w-[72%] grid-cols-2 gap-4">{rows.map(([name,value,flow],index)=><div key={name} className="rounded-xl border border-white/[.08] bg-[var(--airport-panel-alt)]/90 p-4"><div className="flex items-center justify-between"><span className="text-[12px] font-semibold text-white">{name}</span><span className={`h-2 w-2 rounded-full ${index===1?"bg-amber-300":"bg-emerald-300"}`}/></div><div className="mt-3 flex justify-between text-[12px]"><span className="text-cyan-200">{value}</span><span className="text-slate-500">{flow}</span></div></div>)}</div><div className="flex items-center justify-between"><SchematicNode label={vi?"Lịch vận hành":"Schedule"} value="OCCUPIED"/><SchematicLine/><SchematicNode label={vi?"Cảnh báo":"Alarm"} value="1 ACTIVE" warning/><SchematicLine/><SchematicNode label={vi?"Historian":"Historian"} value="6 SEC"/></div></div></div><AirportPanel title={vi?"Điểm dữ liệu schematic":"Schematic data points"}><div className="space-y-2 p-3">{equipment.parameters.map(parameter=><div key={parameter.key} className="flex items-center justify-between rounded-lg bg-white/[.03] px-3 py-2.5"><span className="text-[12px] text-slate-400">{parameter.label}</span><div className="flex items-center gap-2"><b className="text-[12px] text-white">{String(parameter.value)} {parameter.unit}</b><AirportStatusBadge status={statusFromParameter(parameter)}/></div></div>)}</div></AirportPanel></div>;
}

function SchematicNode({label,value,primary=false,warning=false}:{label:string;value:string;primary?:boolean;warning?:boolean}){return <div className={`min-w-[160px] rounded-xl border p-4 text-center ${warning?"border-amber-400/25 bg-amber-400/[.08]":primary?"border-cyan-300/35 bg-cyan-400/[.1]":"border-white/[.08] bg-[var(--airport-panel-alt)]"}`}><p className="text-[12px] font-semibold text-white">{label}</p><p className={`mt-2 text-[11px] ${warning?"text-amber-200":"text-emerald-300"}`}>{value}</p></div>}
function SchematicLine(){return <div className="relative mx-2 h-[2px] flex-1 bg-cyan-400/25"><span className="absolute right-0 top-1/2 -translate-y-1/2 text-[11px] text-cyan-300">▶</span></div>}
...
function ControlView({ equipment, enabled, setEnabled, setpoint, setSetpoint, speed, setSpeed }: { equipment: BMSEquipment; enabled: boolean; setEnabled: (v:boolean)=>void; setpoint:number; setSetpoint:(v:number)=>void; speed:number; setSpeed:(v:number)=>void }) {
  ...
  return <div className="grid gap-4 p-4 xl:grid-cols-[.9fr_1.1fr]"><AirportPanel title={vi?"Điều khiển EBO":"EBO control"} subtitle={vi?"Mọi thao tác trong bản demo đều có phê duyệt và nhật ký":"All demo commands are approval-controlled and audited"}><div className="space-y-4 p-4"><div className="flex items-center justify-between rounded-xl border border-white/[.08] bg-white/[.03] p-4"><div><p className="text-[12px] text-slate-500">{vi?"Trạng thái thiết bị":"Equipment state"}</p><p className="mt-1 text-sm font-semibold text-white">{enabled?(vi?"Đang chạy":"Running"):(vi?"Đã dừng":"Stopped")}</p></div><button onClick={()=>setEnabled(!enabled)} className={`relative h-7 w-14 rounded-full transition ${enabled?"bg-cyan-300":"bg-slate-700"}`}><span className={`absolute top-1 h-5 w-5 rounded-full bg-white transition ${enabled?"left-8":"left-1"}`}/></button></div><ControlSlider label={equipment.id==="chiller"?(vi?"Nhiệt độ nước cấp":"Supply water setpoint"):(vi?"Nhiệt độ đặt":"Temperature setpoint")} value={setpoint} min={equipment.id==="chiller"?4:16} max={equipment.id==="chiller"?12:28} unit="°C" onChange={setSetpoint}/><ControlSlider label={vi?"Tốc độ / công suất":"Speed / capacity"} value={speed} min={20} max={100} unit="%" onChange={setSpeed}/><label className="block"><span className="text-[12px] text-slate-500">{vi?"Chế độ vận hành":"Operating mode"}</span><select className="airport-select mt-2 w-full"><option>Auto / EBO Schedule</option><option>Manual Override</option><option>Energy Optimization</option><option>Maintenance</option></select></label><button onClick={apply} className="airport-button w-full justify-center !border-cyan-400/30 !bg-cyan-400/10 !text-cyan-200"><Power size={14}/>{vi?"Áp dụng lệnh mô phỏng":"Apply simulated command"}</button></div></AirportPanel><AirportPanel title={vi?"Liên động và bảo vệ":"Interlocks and protection"}><div className="space-y-2 p-3">{[[vi?"Cho phép từ lịch EBO":"EBO schedule enable","normal"],[vi?"Liên động bơm / quạt":"Pump / fan interlock","normal"],[vi?"Bảo vệ nhiệt độ":"Temperature protection","normal"],[vi?"Mất lưu lượng":"Loss of flow","normal"],[vi?"Ghi đè đang hoạt động":"Active override","warning"],[vi?"Quyền điều khiển":"Control permission","normal"]].map(([label,status],index)=><div key={label} className="flex items-center justify-between rounded-lg border border-white/[.06] bg-white/[.025] p-3"><span className="text-[12px] text-slate-300">{label}</span><AirportStatusBadge status={status as "normal"|"warning"} label={index===4?(vi?"7 phút còn lại":"7 min left"):(vi?"Sẵn sàng":"Ready")}/></div>)}</div><div className="border-t border-white/[.07] p-4"><p className="text-[11px] uppercase tracking-[.12em] text-slate-600">{vi?"Nhật ký lệnh gần nhất":"Recent command audit"}</p><div className="mt-3 space-y-2 text-[12px] text-slate-400"><p>10:38:12 · Setpoint {setpoint}°C · Demo operator</p><p>10:21:04 · Mode AUTO · EBO Schedule</p><p>09:58:42 · Alarm acknowledged · HVAC Ops</p></div></div></AirportPanel></div>;
}
function ControlSlider({label,value,min,max,unit,onChange}:{label:string;value:number;min:number;max:number;unit:string;onChange:(v:number)=>void}){return <label className="block rounded-xl border border-white/[.07] bg-white/[.025] p-4"><span className="flex justify-between text-[12px] text-slate-500"><span>{label}</span><b className="text-white">{value}{unit}</b></span><input type="range" min={min} max={max} value={value} onChange={e=>onChange(Number(e.target.value))} className="mt-4 w-full accent-cyan-300"/></label>}

function BmsFloorPlan(){const {language}=useAirportLanguage();const vi=language==="vi";const[mode,setMode]=useState<"2d"|"3d">("2d");return <AirportPanel title={vi?"Mặt bằng HVAC Trung tâm R&D":"R&D Center HVAC floor plan"} subtitle={vi?"Chuyển đổi giữa mặt bằng 2D và model 3D được kế thừa từ hệ BMS trước":"Switch between the 2D plan and inherited BMS 3D floor model"} action={<div className="flex rounded-lg border border-white/[.08] bg-white/[.03] p-1">{(["2d","3d"] as const).map(item=><button key={item} onClick={()=>setMode(item)} className={`rounded px-3 py-1.5 text-[11px] font-bold uppercase ${mode===item?"bg-cyan-300 text-[var(--airport-accent-ink)]":"text-slate-500"}`}>{item}</button>)}</div>}><div className="h-[650px] p-3">{mode==="3d"?<BMSModelViewer modelUrl={FLOORPLAN_3D_MODEL_URL} accentColor="#22d3ee"/>:<TerminalFloorPlan2D/>}</div></AirportPanel>}
function TerminalFloorPlan2D(){return <div className="relative h-full overflow-hidden rounded-xl border border-cyan-400/15 bg-[var(--airport-panel)]"><div className="absolute inset-[8%] rounded-[42px] border-2 border-cyan-400/25 bg-cyan-400/[.025]"><div className="absolute left-[8%] right-[8%] top-[18%] h-[18%] rounded-2xl border border-white/[.08] bg-[var(--airport-panel-alt)]"><span className="absolute left-4 top-3 text-[12px] font-semibold text-white">R&D LABS · CLEANROOM SUPPORT</span></div><div className="absolute bottom-[14%] left-[8%] top-[42%] w-[25%] rounded-2xl border border-white/[.08] bg-[var(--airport-panel-alt)]"><span className="absolute left-4 top-3 text-[12px] font-semibold text-white">LAB SUPPORT · ZONE B</span></div><div className="absolute bottom-[14%] left-[37%] right-[8%] top-[42%] rounded-2xl border border-white/[.08] bg-[var(--airport-panel-alt)]"><span className="absolute left-4 top-3 text-[12px] font-semibold text-white">INNOVATION WORKSPACE</span></div>{[[20,26,"AHU-01"],[27,63,"FCU-18"],[51,58,"VAV-05"],[72,58,"VAV-12"],[82,28,"AHU-04"]].map(([x,y,name])=><button key={String(name)} onClick={()=>toast.info(String(name))} className="absolute -translate-x-1/2 -translate-y-1/2 rounded-full border border-cyan-300/40 bg-cyan-300/15 p-2 text-[10px] font-semibold text-cyan-100 shadow-[0_0_18px_rgba(34,211,238,.25)]" style={{left:`${x}%`,top:`${y}%`}}>{name}</button>)}</div><div className="absolute bottom-4 left-4 rounded-lg border border-white/[.08] bg-[var(--airport-panel)]/90 px-3 py-2 text-[11px] text-slate-400">R&D Center · Level 2 · HVAC points live</div></div>}

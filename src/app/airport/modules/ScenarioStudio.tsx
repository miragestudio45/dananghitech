    ...
    <div className="grid gap-4 xl:grid-cols-[.68fr_1.32fr]">
      <AirportPanel title="Scenario definition" subtitle={config.subtitle}>
        <div className="space-y-4 p-4">
          <div className="rounded-xl border border-violet-400/15 bg-violet-400/[.045] p-4">
            <div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-violet-400/10 text-violet-300">{config.icon}</span><div><p className="text-[11px] uppercase tracking-[.14em] text-violet-300">What-if scenario</p><h3 className="mt-1 text-sm font-semibold text-white">{config.title}</h3></div></div>
            <p className="mt-3 text-[12px] leading-relaxed text-slate-400">{config.description}</p>
          </div>

          {config.controls.map((control) => <label key={control.key} className="block rounded-xl border border-white/[.06] bg-white/[.025] p-3">
            <div className="flex items-center justify-between"><span className="text-[12px] text-slate-300">{control.label}</span><b className="text-[12px] text-cyan-200">{control.format(inputs[control.key])}</b></div>
            ...
            <div className="mt-1 flex justify-between text-[10px] text-slate-700"><span>{control.format(control.min)}</span><span>{control.format(control.max)}</span></div>
          </label>)}
          ...
          <button onClick={() => setResponseEnabled((value) => !value)} className={`flex w-full items-center justify-between rounded-xl border p-3 text-left ${responseEnabled ? "border-emerald-400/20 bg-emerald-400/[.055]" : "border-white/[.07] bg-white/[.025]"}`}>
            <div><p className="text-[12px] font-semibold text-white">Optimized response package</p><p className="mt-1 text-[11px] text-slate-500">Compare baseline impact against recommended response actions.</p></div>
          ...
          <p className="text-center text-[10px] leading-relaxed text-slate-600">Demonstration decision-support model. Production use requires calibrated hydraulic, dispersion and utility-network models.</p>
        </div>
      </AirportPanel>
      ...
      <div className="space-y-4">
        ...
        <div className="grid gap-4 xl:grid-cols-2">
          <AirportPanel title="Baseline vs optimized response"><div className="space-y-4 p-4">{config.comparison.map(([label, baseline, response]) => <div key={label}><div className="flex justify-between text-[11px]"><span className="text-slate-400">{label}</span><span><b className="text-amber-200">{baseline}</b><span className="mx-2 text-slate-700">→</span><b className="text-emerald-300">{response}</b></span></div><div className="mt-2 grid grid-cols-2 gap-1"><div className="h-2 overflow-hidden rounded bg-white/[.06]"><div className="h-full bg-amber-400" style={{ width: `${config.baselineBars[label] ?? 80}%` }} /></div><div className="h-2 overflow-hidden rounded bg-white/[.06]"><div className="h-full bg-emerald-400" style={{ width: `${config.responseBars[label] ?? 45}%` }} /></div></div></div>)}</div></AirportPanel>
          <AirportPanel title="Recommended governed actions"><div className="space-y-2 p-3">{config.actions.map((action, index) => <div key={action} className="rounded-lg border border-violet-400/12 bg-violet-400/[.04] p-3"><div className="flex items-start gap-3"><span className="grid h-7 w-7 flex-none place-items-center rounded-full bg-violet-400/10 text-[11px] font-bold text-violet-300">{index + 1}</span><div className="flex-1"><p className="text-[11px] leading-relaxed text-slate-300">{action}</p><p className="mt-1 text-[10px] text-slate-600">Human approval · audit · rollback available</p></div><ShieldCheck size={14} className="text-emerald-300"/></div></div>)}</div></AirportPanel>
        </div>
      </div>
    </div>
  </>;

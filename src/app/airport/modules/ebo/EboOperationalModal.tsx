    ...
    <AnimatePresence>
      {open && <>
        <motion.button
          ...
          onClick={onClose}
          className="fixed bottom-0 left-0 right-0 top-[72px] z-[150] cursor-default bg-black/72 backdrop-blur-sm"
        />
        <motion.section
          role="dialog"
          aria-modal="true"
            ...
            <aside className="airport-scrollbar min-h-0 overflow-y-auto border-l border-white/[.08] bg-[var(--airport-panel-alt)]/72 p-4">
              ...
              <div className="mt-3 space-y-2">
                {fields.map((field) => <div key={field.label} className="rounded-lg border border-white/[.07] bg-white/[.03] px-3 py-2.5">
                  <div className="flex items-start justify-between gap-3"><span className="text-[11px] text-slate-500">{field.label}</span><b className={`text-right text-[12px] ${field.tone === "warning" ? "text-amber-200" : field.tone === "critical" ? "text-red-200" : field.tone === "normal" ? "text-emerald-300" : "text-cyan-200"}`}>{field.value}</b></div>
                </div>)}
              </div>
              <div className="mt-5 rounded-xl border border-emerald-400/15 bg-emerald-400/[.045] p-3">
                <div className="flex items-start gap-2"><CheckCircle2 size={14} className="mt-0.5 flex-none text-emerald-300"/><p className="text-[11px] leading-relaxed text-slate-400">Commands in this demonstration environment require role-based permission, confirmation and an audit-trail entry. Integrated security and video functions remain governed by their source platforms.</p></div>
              </div>
            </aside>
          </div>

...
const SYSTEM_HEALTH = [
  ["BMS / HVAC", "4,286 devices", "99.2%", "18 alarms"],
  ["Power / EMS", "8,420 meters", "99.9%", "7 alarms"],
  ["CCTV / VMS", "1,284 cameras", "99.1%", "12 degraded"],
  ["Access Control", "842 doors", "99.3%", "6 offline"],
  ["Smart Parking", "4,860 spaces", "98.8%", "34 sensors"],
  ["Fire & Life Safety", "8,424 devices", "99.95%", "2 supervisory"],
  ["Water & Utilities", "1,280 points", "99.7%", "4 warnings"],
  ["Lighting", "12,860 circuits", "99.6%", "28 lamps"],
  ["Vertical Transport", "186 units", "98.9%", "3 service"],
] as const;
  ...
  return <>
    <div className="grid grid-cols-3 gap-2 xl:grid-cols-6">
      {[
        ...
        [vi ? "Điểm dữ liệu trực tiếp" : "Live data points", "124,860", "6 sec freshness"],
        [vi ? "Thiết bị đang vận hành" : "Operating devices", "41,268", "98.9% online"],
        [vi ? "Cảnh báo đang mở" : "Open alarms", "104", "7 high priority"],
        [vi ? "Lệnh điều khiển hôm nay" : "Commands today", "2,486", "100% audited"],
    ...
    <div className="grid gap-4 xl:grid-cols-[1.28fr_.72fr]">
      <AirportPanel title={vi ? "Bức tranh vận hành cơ sở vật chất Khu Công nghệ cao" : "High-tech park facility operations picture"} subtitle={vi ? "EBO hợp nhất BMS, EMS, CCTV, ACS, Parking và các hệ kỹ thuật" : "EBO unifies BMS, EMS, CCTV, ACS, parking and technical systems"}>
        <div className="grid grid-cols-3 gap-3 p-4">
          {SYSTEM_HEALTH.map(([name, scope, availability, alarms], index) => <button key={name} onClick={() => toast.info(`${name} · ${scope}`)} className="rounded-xl border border-white/[.07] bg-white/[.025] p-4 text-left transition hover:border-cyan-400/20 hover:bg-cyan-400/[.045]">
            ...
            <div className="mt-3 flex items-center justify-between text-[11px]"><span className="text-emerald-300">{availability}</span><span className="text-slate-500">{alarms}</span></div>
          </button>)}
        </div>
      </AirportPanel>
      <AirportPanel title={vi ? "Tình trạng nền tảng EBO" : "EBO platform posture"}>
        <div className="space-y-2 p-3">
          ...
          ].map(([name, value, status]) => <div key={name} className="flex items-center justify-between rounded-lg border border-white/[.06] bg-white/[.025] p-3"><span className="text-[12px] text-slate-300">{name}</span><div className="flex items-center gap-2"><b className="text-[12px] text-white">{value}</b><AirportStatusBadge status={status as "normal" | "warning"}/></div></div>)}
        </div>
      </AirportPanel>
    </div>
    ...
    <div className="grid gap-4 xl:grid-cols-[1fr_1fr]">
      ...
      <AirportPanel title={vi ? "Sự kiện liên hệ thống" : "Cross-system events"}><div className="p-3"><AirportTimeline events={[
        ["11:42","BMS","AHU R&D-L2 CO₂ above comfort threshold","R&D Center","Warning"],
        ["11:38","EMS","Utility Plant demand peak forecast","12.8 MW","Warning"],
        ["11:31","CCTV","Camera P07 analytics correlated with radar","Perimeter","Critical"],
        ["11:24","ACS","Restricted production-zone credential denied","Cleanroom Gate CR-04","Info"],
        ["11:16","Parking","Logistics parking occupancy reached 88%","Logistics Hub","Warning"],
      ]} /></div></AirportPanel>
    </div>
  </>;
}
  ...
  return <>
    <div className="grid grid-cols-3 gap-2 xl:grid-cols-6">
      {[
        ...
        [vi ? "Điểm đang quét" : "Scanning points", "124,860", "6 sec"], [vi ? "Lịch vận hành" : "Active schedules", "486", "Park-wide"],
    ...
    <div className="grid min-h-[690px] gap-4 xl:grid-cols-[280px_minmax(0,1fr)_320px]">
      ...
      <AirportPanel title={`${active} · ${vi ? "Không gian điều hành" : "Operational workspace"}`} subtitle={vi ? "Giám sát, xu hướng, lệnh và trạng thái theo thời gian thực" : "Live monitoring, trends, commands and state"} className="h-full">
        <div className="grid h-full min-h-[620px] grid-rows-[auto_1fr_auto] gap-3 p-4">
          <div className="grid grid-cols-4 gap-2">{[["Availability","99.2%"],["Active points","18,420"],["Open alarms","18"],["Control mode","AUTO"]].map(([a,b], i) => <div key={a} className="rounded-lg border border-white/[.07] bg-white/[.03] p-3"><p className="text-[11px] text-slate-500">{a}</p><p className={`mt-1 text-sm font-semibold ${i === 2 ? "text-amber-200" : "text-white"}`}>{b}</p></div>)}</div>
          <div className="rounded-xl border border-white/[.07] bg-[var(--airport-panel)] p-3"><AirportTrendChart data={trend} height={360} color="#22d3ee" unit="%" /></div>
          <div className="grid grid-cols-4 gap-2">{["Acknowledge alarms","Open schematic","Create work order","Export event log"].map((label, i) => <button key={label} onClick={() => toast.success(`${label} · demo`)} className={`rounded-lg border p-3 text-[11px] font-semibold ${i === 0 ? "border-amber-400/20 bg-amber-400/[.07] text-amber-200" : "border-cyan-400/15 bg-cyan-400/[.045] text-cyan-200"}`}>{label}</button>)}</div>
        </div>
      </AirportPanel>
      <AirportPanel title={vi ? "Live alarms & commands" : "Live alarms & commands"} className="h-full">
        <div className="space-y-2 p-3">{[
          ...
          ["11:31","Critical","CAM-P07","Perimeter correlation"],["11:24","Info","ACS-A04","Credential denied"],
          ["11:16","Medium","PARK-P2","Occupancy 88%"],["11:09","Low","LGT-T1-18","Lamp group degraded"],
        ].map(([time, priority, asset, message]) => <button key={`${time}-${asset}`} onClick={() => toast.info(`${asset}: ${message}`)} className="w-full rounded-lg border border-white/[.06] bg-white/[.025] p-3 text-left"><div className="flex items-center justify-between"><span className="font-mono text-[11px] text-cyan-300">{time}</span><AirportStatusBadge status={priority === "Critical" ? "critical" : priority === "High" || priority === "Medium" ? "warning" : "info"} label={priority}/></div><p className="mt-2 text-[12px] font-semibold text-white">{asset}</p><p className="mt-1 text-[11px] text-slate-500">{message}</p></button>)}</div>
      </AirportPanel>
    </div>
  </>;
}
...
function EnergyPowerOperations() {
  ...
  return <>
    ...
    <div className="grid gap-4 xl:grid-cols-[1.25fr_.75fr]">
      <AirportPanel title={vi?"Sơ đồ điện một sợi Khu Công nghệ cao":"High-tech park single-line power diagram"} action={<select value={source} onChange={e=>setSource(e.target.value)} className="airport-select"><option>Grid A</option><option>Grid B</option><option>Generator Bus</option><option>UPS Bus</option></select>}>
        <div className="relative min-h-[520px] overflow-hidden p-5" style={{backgroundImage:"radial-gradient(circle at 1px 1px,rgba(34,211,238,.12) 1px,transparent 0)",backgroundSize:"22px 22px"}}>
          ...
          ].map(([name,value,status],index)=><React.Fragment key={name}><button onClick={()=>setSelected({name,value,state:status})} className={`rounded-xl border p-4 text-center ${index===4?"border-amber-400/25 bg-amber-400/[.08]":"border-cyan-400/18 bg-cyan-400/[.055]"}`}><Zap className="mx-auto text-cyan-300" size={20}/><p className="mt-3 text-[12px] font-semibold text-white">{name}</p><p className="mt-2 text-[11px] text-emerald-300">{value}</p><AirportStatusBadge status={status as "normal"|"warning"}/></button>{index<4&&<div className="h-[2px] bg-cyan-400/30"><span className="float-right -mt-2 text-cyan-300">▶</span></div>}</React.Fragment>)}</div>
        </div>
      </AirportPanel>
      <AirportPanel title={vi?"Nguồn và phụ tải trọng yếu":"Sources and critical loads"}><div className="space-y-2 p-3">{sources.map((item,i)=><button key={item.name} onClick={()=>setSelected(item)} className="w-full rounded-lg border border-white/[.06] bg-white/[.025] p-3 text-left"><div className="flex items-center justify-between"><p className="text-[12px] font-semibold text-white">{item.name}</p><AirportStatusBadge status={i===2?"info":"normal"} label={item.state}/></div><p className="mt-2 text-[11px] text-slate-500">{item.value}</p></button>)}</div></AirportPanel>
    </div>
    ...
    <EboOperationalModal open={Boolean(selected)} onClose={()=>setSelected(null)} title={selected?.name ?? "Power asset"} subtitle="PME / EBO integrated electrical context" status={selected?.state==="warning"?"warning":"normal"} statusLabel={selected?.state ?? "Online"} fields={[
      ...
      <div className="grid gap-4 xl:grid-cols-[1.2fr_.8fr]"><AirportPanel title="Electrical operating profile"><div className="p-3"><AirportTrendChart data={makeTrend(5,72,12)} height={330} color="#fbbf24" unit="%"/></div></AirportPanel><AirportPanel title="Protection and switching state"><div className="space-y-2 p-3">{[["Breaker state","Closed"],["Protection relay","Healthy"],["Power quality","Within limits"],["Control authority","Authorized operator only"],["Last command","12:39:18 · audited"]].map(([a,b])=><div key={a} className="flex justify-between rounded-lg border border-white/[.06] bg-white/[.03] p-3 text-[12px]"><span className="text-slate-500">{a}</span><b className="text-white">{b}</b></div>)}</div></AirportPanel></div>
    </EboOperationalModal>
  </>;
}
...
function CctvVmsOperations() {
  const { language }=useAirportLanguage();
  const vi=language==="vi";
  const cameras = CCTV_STOCK_MEDIA.map((media,i)=>({id:`CAM-${String(i+1).padStart(3,"0")}`,zone:["R&D Center","Manufacturing Precinct","Perimeter","Logistics Hub"][i%4],status:[3,9].includes(i)?"Degraded":"Live",event:[2,7].includes(i)?"Analytics event":"Normal",resolution:i%3===0?"4K":"1080p",retention:"30 days",...media}));
  const [selected,setSelected]=useState<(typeof cameras)[number]|null>(null);
  const [playing,setPlaying]=useState(true);
  return <>
    <div className="grid grid-cols-3 gap-2 xl:grid-cols-6">{[[vi?"Camera trực tuyến":"Cameras online","1,272 / 1,284","99.1%"],[vi?"Luồng AI":"AI analytics streams","428","Live"],[vi?"Sự kiện hôm nay":"Events today","2,846","-6%"],[vi?"Camera suy giảm":"Degraded cameras","12","Maintenance"],[vi?"Lưu trữ":"Retention","30 days","Healthy"],[vi?"Độ phủ":"Coverage confidence","98.4%","Park-wide"]].map(([a,b,c],i)=><AirportMetricCard key={a} label={a} value={b} trend={c} compact tone={i===3?"amber":"cyan"}/>)}</div>
    <div className="grid gap-4 xl:grid-cols-[1.35fr_.65fr]">
      <AirportPanel title={vi?"Video wall VMS":"VMS video wall"}><div className="grid grid-cols-4 gap-2 p-3">{cameras.map((cam)=><button key={cam.id} onClick={()=>{setSelected(cam);setPlaying(true);}} className="relative aspect-video overflow-hidden rounded-lg border border-white/[.07] bg-[var(--airport-sidebar)]"><img src={cam.posterUrl} alt={`${cam.id} · ${cam.label}`} loading="lazy" className="pointer-events-none absolute inset-0 h-full w-full object-cover grayscale contrast-125 saturate-50"/><div className="pointer-events-none absolute inset-0 opacity-20" style={{backgroundImage:"repeating-linear-gradient(0deg,transparent 0,transparent 3px,rgba(34,211,238,.22) 4px)"}}/><div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-[var(--airport-panel)]/85 via-transparent to-black/25"/><span className="absolute left-2 top-2 rounded bg-black/55 px-1.5 py-1 font-mono text-[10px] text-cyan-100/80">{cam.label}</span><div className="absolute inset-x-0 bottom-0 flex items-center justify-between bg-[var(--airport-panel)]/75 px-2 py-1.5"><span className="font-mono text-[10px] text-white">{cam.id}</span><span className={`h-1.5 w-1.5 rounded-full ${cam.status==="Live"?"bg-emerald-300":"bg-amber-300"}`}/></div>{cam.event!=="Normal"&&<span className="absolute right-2 top-2 rounded bg-amber-400/15 px-1.5 py-1 text-[10px] text-amber-200">AI EVENT</span>}</button>)}</div></AirportPanel>
      <AirportPanel title={vi?"Sự kiện video ưu tiên":"Priority video events"}><div className="space-y-2 p-3">{[["11:31","Perimeter P-07","Intrusion correlation","97%"],["11:18","Main Gate","Crowd density","86%"],["10:56","Cleanroom Lobby","Unattended object","Review"],["10:42","Logistics Yard North","Vehicle route deviation","Closed"],["10:24","Warehouse Gate 04","Tailgating","Investigating"]].map(([time,zone,event,confidence],i)=><button key={`${time}-${zone}`} className="w-full rounded-lg border border-white/[.06] bg-white/[.025] p-3 text-left" onClick={()=>setSelected(cameras[i])}><div className="flex justify-between"><span className="font-mono text-[11px] text-cyan-300">{time}</span><AirportStatusBadge status={i===0?"critical":i<3?"warning":"info"}/></div><p className="mt-2 text-[12px] font-semibold text-white">{zone}</p><p className="mt-1 text-[11px] text-slate-500">{event} · {confidence}</p></button>)}</div></AirportPanel>
    </div>
    <EboOperationalModal open={Boolean(selected)} onClose={()=>setSelected(null)} eyebrow="VMS integrated operational view" title={`${selected?.id ?? "Camera"} · ${selected?.zone ?? ""}`} subtitle="Video remains governed by the source VMS; EBO presents linked status, events and workflow." status={selected?.status==="Degraded"?"warning":"normal"} statusLabel={selected?.status ?? "Live"} fields={[
      {label:"Camera ID",value:selected?.id ?? "—"},{label:"Zone",value:selected?.zone ?? "—"},{label:"Stream",value:selected?.resolution ?? "—",tone:"normal"},{label:"Analytics",value:selected?.event ?? "—",tone:selected?.event!=="Normal"?"warning":"normal"},{label:"Retention",value:selected?.retention ?? "—"},{label:"Source platform",value:"High-Tech Park VMS"},
      ...
      <div className="grid gap-4 xl:grid-cols-[1.3fr_.7fr]"><div className="relative min-h-[520px] overflow-hidden rounded-xl border border-cyan-400/18 bg-[var(--airport-sidebar)]"><CctvStockVideo src={selected?.mediaUrl ?? CCTV_STOCK_MEDIA[0].mediaUrl} playing={playing} className="absolute inset-0 h-full w-full object-cover grayscale-[20%] contrast-125 saturate-75"/><div className="pointer-events-none absolute inset-0 opacity-15" style={{backgroundImage:"repeating-linear-gradient(0deg,transparent 0,transparent 4px,rgba(34,211,238,.2) 5px)"}}/><div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-black/45 via-transparent to-black/20"/><div className="pointer-events-none absolute left-3 top-3 flex items-center gap-2 rounded-lg bg-black/55 px-3 py-2 font-mono text-[11px] text-white"><span className={`h-2 w-2 rounded-full ${playing?"bg-red-400":"bg-slate-500"}`}/>{playing?"REC":"PAUSED"} · {selected?.id} · {selected?.label}</div><div className="pointer-events-none absolute bottom-3 left-3 right-3 flex items-center justify-between rounded-xl border border-white/[.08] bg-black/50 px-3 py-2"><span className="font-mono text-[11px] text-slate-300">12:41:28 · UTC+7</span><span className="text-[11px] text-cyan-200">AI analytics overlay active</span></div></div><AirportPanel title="PTZ, analytics and event history"><div className="grid grid-cols-3 gap-2 p-3">{["Pan left","Tilt up","Zoom +","Home","Auto track","Privacy mask"].map(label=><button key={label} onClick={()=>toast.success(`${label} · demo command audited`)} className="rounded-lg border border-white/[.07] bg-white/[.03] p-3 text-[11px] text-slate-300 hover:border-cyan-400/20 hover:text-cyan-200">{label}</button>)}</div><div className="space-y-2 border-t border-white/[.06] p-3">{[["12:39:18","Motion analytics","Normal"],["12:31:04","Object classification","Person"],["12:16:42","Stream health","Recovered"],["11:58:11","Operator view","Opened"]].map(([time,event,state])=><div key={time} className="flex justify-between rounded-lg bg-white/[.03] p-3 text-[11px]"><span className="font-mono text-cyan-300">{time}</span><span className="text-slate-300">{event}</span><span className="text-emerald-300">{state}</span></div>)}</div></AirportPanel></div>
    </EboOperationalModal>
  </>;
}
...
function AccessControlOperations(){
  ...
  const vi=language==="vi";
  const rows=[
    {id:"ACS-T1-001",name:"R&D Center main entry",state:"Online",events:"12,840",denied:"0",zone:"Public / Logistics Hub",policy:"Open policy"},
  ...
  return <><div className="grid grid-cols-3 gap-2 xl:grid-cols-6">{[[vi?"Điểm kiểm soát":"Access points","842 / 848","99.3%"],[vi?"Thẻ hợp lệ":"Valid credentials","12,480","Park-wide"],[vi?"Lượt vào hôm nay":"Access today","186,420","+4.1%"],[vi?"Từ chối":"Denied events","42","Review"],[vi?"Cửa mở lâu":"Held-open doors","3","Action"],[vi?"Anti-passback":"Anti-passback","99.8%","Healthy"]].map(([a,b,c],i)=><AirportMetricCard key={a} label={a} value={b} trend={c} compact tone={i===3||i===4?"amber":"cyan"}/>)}</div><div className="grid gap-4 xl:grid-cols-[1.25fr_.75fr]"><AirportPanel title={vi?"Trạng thái cửa và đầu đọc":"Door and reader status"}><div className="overflow-auto"><table className="w-full min-w-[760px] text-left text-[12px]"><thead className="bg-[var(--airport-panel-strong)] text-[10px] uppercase tracking-[.11em] text-slate-500"><tr>{["ID","Access point","State","Events today","Denied"].map(h=><th key={h} className="px-3 py-2">{h}</th>)}</tr></thead><tbody>{rows.map(row=><tr key={row.id} onClick={()=>setSelected(row)} className="cursor-pointer border-t border-white/[.05] text-slate-300 hover:bg-cyan-400/[.04]"><td className="px-3 py-3 font-mono text-cyan-300">{row.id}</td><td>{row.name}</td><td><AirportStatusBadge status={row.state==="Online"?"normal":"warning"} label={row.state}/></td><td>{row.events}</td><td>{row.denied}</td></tr>)}</tbody></table></div></AirportPanel><AirportPanel title={vi?"Quản trị vùng truy cập":"Access-zone governance"}><div className="space-y-2 p-3">{rows.map((row,i)=><button key={row.id} onClick={()=>setSelected(row)} className="w-full rounded-lg border border-white/[.06] bg-white/[.025] p-3 text-left"><div className="flex justify-between"><p className="text-[12px] font-semibold text-white">{row.zone}</p><AirportStatusBadge status={i===2?"warning":"normal"}/></div><p className="mt-2 text-[11px] text-slate-500">{row.policy}</p></button>)}</div></AirportPanel></div>
...
function FireLifeSafetyOperations(){
  ...
  return <><div className="grid grid-cols-3 gap-2 xl:grid-cols-6">{[[vi?"Tủ báo cháy":"Fire panels","18 / 18","Normal"],[vi?"Thiết bị online":"Devices online","8,420 / 8,424","99.95%"],[vi?"Tín hiệu giám sát":"Supervisory","2","Review"],[vi?"Zone sẵn sàng":"Zones ready","42 / 42","Healthy"],[vi?"Hệ chữa cháy":"Suppression systems","26 / 26","Armed"],[vi?"Sẵn sàng sơ tán":"Evacuation readiness","98.7%","Healthy"]].map(([a,b,c],i)=><AirportMetricCard key={a} label={a} value={b} trend={c} compact tone={i===2?"amber":i===5?"emerald":"cyan"}/>)}</div><div className="grid gap-4 xl:grid-cols-[1.15fr_.85fr]"><AirportPanel title={vi?"Bản đồ zone PCCC":"Fire-zone readiness map"}><div className="grid grid-cols-7 gap-2 p-4">{zones.map(zone=><button key={zone.id} onClick={()=>setSelected(zone)} className={`aspect-square rounded-lg border p-2 text-center ${zone.state!=="Normal"?"border-amber-400/25 bg-amber-400/[.08]":"border-emerald-400/16 bg-emerald-400/[.045]"}`}><Flame size={13} className={`mx-auto ${zone.state!=="Normal"?"text-amber-300":"text-emerald-300"}`}/><span className="mt-1 block text-[10px] text-white">{zone.id}</span></button>)}</div></AirportPanel><AirportPanel title={vi?"Tín hiệu và quy trình":"Signals and response workflow"}><div className="space-y-2 p-3">{signals.map(item=><button key={item.id} onClick={()=>setSelected(item)} className="w-full rounded-lg border border-white/[.06] bg-white/[.025] p-3 text-left"><div className="flex justify-between"><span className="font-mono text-[11px] text-cyan-300">{item.id}</span><AirportStatusBadge status={item.state==="Warning"?"warning":"normal"}/></div><p className="mt-2 text-[12px] text-slate-300">{item.message}</p></button>)}</div></AirportPanel></div>
    <EboOperationalModal open={Boolean(selected)} onClose={()=>setSelected(null)} eyebrow="Fire alarm integration view" title={selected?.id ?? "Fire zone"} subtitle="Fire alarm and life-safety status is presented from the certified source system." status={selected?.state==="Normal"?"normal":"warning"} statusLabel={selected?.state ?? "Normal"} fields={[
      ...
      <div className="grid gap-4 xl:grid-cols-[1fr_1fr]"><AirportPanel title="Zone devices and readiness"><div className="grid grid-cols-2 gap-3 p-4">{[["Smoke detectors","168 / 168"],["Heat detectors","42 / 42"],["Manual call points","18 / 18"],["Sounders / strobes","64 / 64"],["Sprinkler valves","12 / 12"],["Stair pressurization","4 / 4"]].map(([a,b])=><div key={a} className="rounded-lg border border-white/[.07] bg-white/[.03] p-4"><p className="text-[11px] text-slate-500">{a}</p><p className="mt-2 text-sm font-semibold text-emerald-300">{b}</p></div>)}</div></AirportPanel><AirportPanel title="Response sequence"><div className="space-y-2 p-4">{[["01","Detect and validate","Complete"],["02","Notify fire command","Ready"],["03","Control smoke / pressurization","Ready"],["04","Voice evacuation by zone","Standby"],["05","Fire brigade handoff","Standby"],["06","Restore and audit","Pending"]].map(([n,a,b])=><div key={n} className="flex items-center gap-3 rounded-lg border border-white/[.06] bg-white/[.025] p-3"><span className="text-[11px] text-cyan-300">{n}</span><span className="flex-1 text-[12px] text-white">{a}</span><span className="text-[11px] text-slate-500">{b}</span></div>)}</div></AirportPanel></div>
    </EboOperationalModal>
  </>;
}
...
function WaterUtilitiesOperations(){
  ...
  return <><div className="grid grid-cols-3 gap-2 xl:grid-cols-6">{[[vi?"Nước cấp hôm nay":"Water supply today","6,840 m³","+3.2%"],[vi?"Bể chứa":"Storage level","78%","Healthy"],[vi?"Áp lực mạng":"Network pressure","4.2 bar","Stable"],[vi?"Nước thải xử lý":"Wastewater treated","5,420 m³","99.1%"],[vi?"Chất lượng":"Water quality","98.8%","Compliant"],[vi?"Rò rỉ dự báo":"Leak anomalies","3","Investigate"]].map(([a,b,c],i)=><AirportMetricCard key={a} label={a} value={b} trend={c} compact tone={i===5?"amber":i===4?"emerald":"cyan"}/>)}</div><div className="grid gap-4 xl:grid-cols-[1.2fr_.8fr]"><AirportPanel title={vi?"Sơ đồ cấp và xử lý nước":"Water supply and treatment schematic"}><div className="flex min-h-[460px] items-center gap-3 overflow-auto p-5">{assets.slice(0,5).map((item,i)=><React.Fragment key={item.name}><button onClick={()=>setSelected(item)} className="min-w-[150px] rounded-xl border border-cyan-400/18 bg-cyan-400/[.05] p-4 text-center"><Droplets className="mx-auto text-cyan-300"/><p className="mt-3 text-[12px] font-semibold text-white">{item.name}</p><p className="mt-2 text-[11px] text-emerald-300">{item.value}</p></button>{i<4&&<div className="h-[2px] min-w-14 bg-cyan-400/30"><span className="float-right -mt-2 text-cyan-300">▶</span></div>}</React.Fragment>)}</div></AirportPanel><AirportPanel title={vi?"Trạm bơm và chất lượng":"Pumps and water quality"}><div className="space-y-2 p-3">{[["Booster Pump BP-01","Running · 78%","Normal"],["Booster Pump BP-02","Standby","Normal"],["Wastewater Train 01","4,280 m³/day","Normal"],["pH","7.2","Normal"],["Turbidity","0.42 NTU","Normal"],["Leak Zone W-07","3.4 L/min anomaly","Warning"]].map(([name,value,state],i)=><button key={name} onClick={()=>setSelected(assets[i===5?5:Math.min(i,4)])} className="w-full rounded-lg border border-white/[.06] bg-white/[.025] p-3 text-left"><div className="flex justify-between"><p className="text-[12px] font-semibold text-white">{name}</p><AirportStatusBadge status={state==="Warning"?"warning":"normal"}/></div><p className="mt-1 text-[11px] text-slate-500">{value}</p></button>)}</div></AirportPanel></div>
    <EboOperationalModal open={Boolean(selected)} onClose={()=>setSelected(null)} title={selected?.name ?? "Water asset"} subtitle="Water, wastewater and leak-detection operational detail" status={selected?.state==="Warning"?"warning":"normal"} statusLabel={selected?.state ?? "Normal"} fields={[
      ...
      <div className="grid gap-4 xl:grid-cols-[1.2fr_.8fr]"><AirportPanel title="Flow, pressure and quality trend"><div className="p-3"><AirportTrendChart data={makeTrend(7,66,11)} height={340} color="#22d3ee" unit="%"/></div></AirportPanel><AirportPanel title="Control and quality points"><div className="space-y-2 p-3">{[["Pump mode","AUTO"],["Discharge pressure","4.2 bar"],["Tank level","78%"],["pH","7.2"],["Turbidity","0.42 NTU"],["Leak model confidence","91.8%"]].map(([a,b])=><div key={a} className="flex justify-between rounded-lg border border-white/[.06] bg-white/[.03] p-3 text-[12px]"><span className="text-slate-500">{a}</span><b className="text-white">{b}</b></div>)}</div></AirportPanel></div>
    </EboOperationalModal>
  </>;
}
...
function LightingOperations(){
  ...
  return <><div className="grid grid-cols-3 gap-2 xl:grid-cols-6">{[[vi?"Mạch chiếu sáng":"Lighting circuits","12,860","99.6%"],[vi?"Đèn đường & công cộng":"Road & public lights","8,420","99.8%"],[vi?"Điện chiếu sáng":"Lighting power","2.84 MW","-6.2%"],[vi?"Lịch đang chạy":"Active schedules","184","Live"],[vi?"Cảm biến hiện diện":"Occupancy sensors","2,486","98.9%"],[vi?"Đèn cần bảo trì":"Maintenance due","28","Action"]].map(([a,b,c],i)=><AirportMetricCard key={a} label={a} value={b} trend={c} compact tone={i===5?"amber":"cyan"}/>)}</div><div className="grid gap-4 xl:grid-cols-[1fr_1fr]"><AirportPanel title={vi?"Zone chiếu sáng và chế độ":"Lighting zones and modes"}><div className="grid grid-cols-2 gap-3 p-4">{zones.map((zone,i)=><button key={zone.name} onClick={()=>{setSelected(zone);setCommandLevel(zone.level);}} className="rounded-xl border border-white/[.07] bg-white/[.025] p-4 text-left"><div className="flex justify-between"><Lightbulb className={i>2?"text-amber-300":"text-cyan-300"} size={20}/><AirportStatusBadge status="normal" label={zone.mode}/></div><p className="mt-3 text-[12px] font-semibold text-white">{zone.name}</p><p className="mt-1 text-xl font-semibold text-white">{zone.level}%</p></button>)}</div></AirportPanel><AirportPanel title={vi?"Lịch và cảm biến":"Schedules and sensors"}><div className="p-3"><AirportTrendChart data={makeTrend(3,68,18)} height={320} color="#fbbf24" unit="%"/></div></AirportPanel></div>
    <EboOperationalModal open={Boolean(selected)} onClose={()=>setSelected(null)} title={selected?.name ?? "Lighting zone"} subtitle="Schedule, occupancy and park-lighting operational context" status="normal" statusLabel={selected?.mode ?? "Auto"} fields={[
      ...
      <div className="grid gap-4 xl:grid-cols-[.85fr_1.15fr]"><AirportPanel title="Lighting control"><div className="p-5"><div className="grid min-h-[300px] place-items-center rounded-xl border border-cyan-400/16 bg-[radial-gradient(circle_at_center,rgba(251,191,36,.16),var(--airport-sidebar)_68%)]"><Lightbulb size={92} className="text-amber-200"/><p className="absolute mt-40 text-3xl font-semibold text-white">{commandLevel}%</p></div><label className="mt-5 block"><div className="flex justify-between text-[12px]"><span className="text-slate-500">Command level</span><b className="text-white">{commandLevel}%</b></div><input type="range" min="0" max="100" value={commandLevel} onChange={e=>setCommandLevel(Number(e.target.value))} className="mt-3 w-full accent-cyan-300"/></label></div></AirportPanel><AirportPanel title="Schedule and trend"><div className="p-3"><AirportTrendChart data={makeTrend(3,68,18)} height={300} color="#fbbf24" unit="%"/></div><div className="grid grid-cols-2 gap-2 p-3">{[["Occupied mode","06:00–23:00"],["After-hours","20%"],["Sensor delay","10 min"],["Manual override","Expires in 30 min"]].map(([a,b])=><div key={a} className="rounded-lg bg-white/[.03] p-3"><p className="text-[11px] text-slate-500">{a}</p><p className="mt-1 text-[12px] text-white">{b}</p></div>)}</div></AirportPanel></div>
    </EboOperationalModal>
  </>;
}
...
function VerticalTransportOperations(){
  ...
  return <><div className="grid grid-cols-3 gap-2 xl:grid-cols-6">{[[vi?"Thang máy":"Elevators","86 / 88","97.7%"],[vi?"Thang cuốn":"Escalators","96 / 98","98.0%"],[vi?"Lượt vận chuyển":"Trips today","84,260","+5.1%"],[vi?"Thiết bị bảo trì":"Maintenance due","7","Next 7d"],[vi?"Sự cố kẹt":"Entrapment","0","Healthy"],[vi?"Thời gian đáp ứng":"Response time","4.8 min","SLA"]].map(([a,b,c],i)=><AirportMetricCard key={a} label={a} value={b} trend={c} compact tone={i===3?"amber":i===4?"emerald":"cyan"}/>)}</div><div className="grid gap-4 xl:grid-cols-[1.2fr_.8fr]"><AirportPanel title={vi?"Đội thiết bị vận chuyển đứng":"Vertical transport fleet"}><div className="grid grid-cols-3 gap-2 p-4">{units.map(unit=><button key={unit.id} onClick={()=>setSelected(unit)} className={`rounded-lg border p-3 text-left ${unit.state==="Service due"?"border-amber-400/25 bg-amber-400/[.08]":"border-cyan-400/15 bg-cyan-400/[.045]"}`}><ArrowUpDown size={18} className={unit.state==="Service due"?"text-amber-300":"text-cyan-300"}/><p className="mt-2 text-[12px] font-semibold text-white">{unit.id}</p><p className="mt-1 text-[11px] text-slate-500">{unit.zone}</p><p className="mt-2 text-[11px] text-emerald-300">{unit.trips.toLocaleString()} trips</p></button>)}</div></AirportPanel><AirportPanel title={vi?"Bảo trì và lưu lượng":"Maintenance and traffic"}><div className="p-3"><AirportTrendChart data={makeTrend(9,72,14)} height={280} color="#a78bfa" unit="%"/></div><div className="grid grid-cols-2 gap-2 p-3">{[["Brake tests","98.4%"],["Door cycles","4.2M"],["Energy recovery","18%"],["SLA","96.8%"]].map(([a,b])=><div key={a} className="rounded-lg bg-white/[.03] p-3"><p className="text-[11px] text-slate-500">{a}</p><p className="mt-1 text-sm font-semibold text-white">{b}</p></div>)}</div></AirportPanel></div>
    <EboOperationalModal open={Boolean(selected)} onClose={()=>setSelected(null)} title={selected?.id ?? "Vertical transport"} subtitle={`${selected?.zone ?? ""} · ${selected?.type ?? ""}`} status={selected?.state==="Service due"?"warning":"normal"} statusLabel={selected?.state ?? "Running"} fields={[
      ...
      <div className="grid gap-4 xl:grid-cols-[.7fr_1.3fr]"><div className="grid min-h-[430px] place-items-center rounded-xl border border-cyan-400/16 bg-[radial-gradient(circle_at_center,#15334b_0,var(--airport-sidebar)_70%)]"><div className="text-center"><ArrowUpDown size={92} className="mx-auto text-cyan-300/70"/><p className="mt-4 text-sm font-semibold text-white">{selected?.id}</p><p className="mt-2 text-[12px] text-emerald-300">Safety circuit healthy</p><p className="mt-1 text-[11px] text-slate-500">Current position {selected?.floor}</p></div></div><AirportPanel title="Operations and maintenance"><div className="p-3"><AirportTrendChart data={makeTrend(9,72,14)} height={270} color="#a78bfa" unit="%"/></div><div className="grid grid-cols-2 gap-2 p-3">{[["Drive current","48 A"],["Door cycles","186,420"],["Brake test","Passed"],["Vibration","2.1 mm/s"],["Next service","18 Jul 2026"],["Last reset","12 Jun 2026"]].map(([a,b])=><div key={a} className="rounded-lg bg-white/[.03] p-3"><p className="text-[11px] text-slate-500">{a}</p><p className="mt-1 text-[12px] text-white">{b}</p></div>)}</div></AirportPanel></div>
    </EboOperationalModal>
  </>;
}
...
const CROSS_DOMAIN_ALARMS = [
  ...
  ["EBO-260713-003","11:31","Critical","CCTV","CAM-P07-04","Perimeter intrusion correlation","Investigating","Park Security"],
  ["EBO-260713-004","11:24","Low","ACS","AIR-GATE-A04","Credential denied","Closed","Access Control"],
  ["EBO-260713-005","11:16","Medium","Parking","PARK-P2","Occupancy reached 88%","Open","Logistics Hub Ops"],

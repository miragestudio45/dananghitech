...
function CctvVmsOperations() {
  ...
  return <>
    ...
    <div className="grid gap-4 xl:grid-cols-[1.35fr_.65fr]">
      <AirportPanel title={vi?"Video wall VMS":"VMS video wall"}><div className="grid grid-cols-4 gap-2 p-3">{cameras.map((cam)=><button key={cam.id} onClick={()=>{setSelected(cam);setPlaying(true);}} className="relative aspect-video overflow-hidden rounded-lg border border-white/[.07] bg-[#06111f]"><img src={cam.posterUrl} alt={`${cam.id} · ${cam.label}`} loading="lazy" className="pointer-events-none absolute inset-0 h-full w-full object-cover grayscale contrast-125 saturate-50"/><div className="pointer-events-none absolute inset-0 opacity-20" style={{backgroundImage:"repeating-linear-gradient(0deg,transparent 0,transparent 3px,rgba(34,211,238,.22) 4px)"}}/><div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-[#04111f]/85 via-transparent to-black/25"/><span className="absolute left-2 top-2 rounded bg-black/55 px-1.5 py-1 font-mono text-[10px] text-cyan-100/80">{cam.label}</span><div className="absolute inset-x-0 bottom-0 flex items-center justify-between bg-[#04111f]/75 px-2 py-1.5"><span className="font-mono text-[10px] text-white">{cam.id}</span><span className={`h-1.5 w-1.5 rounded-full ${cam.status==="Live"?"bg-emerald-300":"bg-amber-300"}`}/></div>{cam.event!=="Normal"&&<span className="absolute right-2 top-2 rounded bg-amber-400/15 px-1.5 py-1 text-[10px] text-amber-200">AI EVENT</span>}</button>)}</div></AirportPanel>

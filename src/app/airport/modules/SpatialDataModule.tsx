...
function TerrainElevation() {
  ...
  return <>
    ...
    <div className="grid gap-4 xl:grid-cols-[1.25fr_.75fr]">
      <AirportPanel title="Terrain and elevation analytical surface" action={<div className="flex rounded-lg border border-white/10 bg-white/[.03] p-1">{(["DTM","DSM"] as const).map((item) => <button key={item} onClick={() => setSurface(item)} className={`rounded px-3 py-1.5 text-[9px] font-bold ${surface === item ? "bg-cyan-300 text-slate-950" : "text-slate-500"}`}>{item}</button>)}</div>}>
        <div className="p-4">
          <div className="relative h-[360px] overflow-hidden rounded-2xl border border-cyan-400/12 bg-gradient-to-b from-[#0a2840] to-[#04111f]">
            <div className="absolute inset-0 opacity-25" style={{ backgroundImage: "linear-gradient(rgba(255,255,255,.14) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,.14) 1px,transparent 1px)", backgroundSize: "42px 42px" }} />
            ...
            <div className="absolute left-4 top-4 rounded-lg border border-white/10 bg-[#06111f]/80 px-3 py-2"><p className="text-[8px] uppercase tracking-[.14em] text-cyan-300">Active surface</p><p className="mt-1 text-sm font-semibold text-white">{surface === "DTM" ? "Bare-earth terrain" : "Buildings and vegetation surface"}</p></div>
            <div className="absolute bottom-4 left-4 right-4 grid grid-cols-4 gap-2">{[["Lowest","4.2 m"],["Median","34.8 m"],["Highest","128 m"],["Flood threshold","18.0 m"]].map(([a,b]) => <div key={a} className="rounded-lg border border-white/10 bg-[#06111f]/75 p-2 backdrop-blur"><p className="text-[8px] text-slate-500">{a}</p><p className="mt-1 text-xs font-semibold text-white">{b}</p></div>)}</div>
          </div>
        </div>
      </AirportPanel>
      <div className="space-y-4">
        <AirportPanel title="Analytical products"><div className="grid gap-2 p-3">{["Elevation and contour","Slope and aspect","Catchment and flow direction","Cut / fill planning","Flood simulation terrain","Protection-corridor checks"].map((item,index)=><div key={item} className="flex items-center gap-3 rounded-lg border border-white/[.06] bg-white/[.025] p-3"><span className="grid h-8 w-8 place-items-center rounded-lg bg-cyan-400/10 text-cyan-300">{index < 2 ? <Mountain size={15}/> : index < 4 ? <Map size={15}/> : <Waves size={15}/>}</span><p className="text-[10px] text-slate-300">{item}</p></div>)}</div></AirportPanel>
        <AirportPanel title="Model confidence"><div className="space-y-3 p-4">{[["Vertical accuracy","±7 cm",96],["Ground classification","98.2%",98],["Hydrology completeness","94.8%",95],["Survey control closure","1:48,000",99]].map(([a,b,v])=><div key={a as string}><div className="flex justify-between text-[9px]"><span className="text-slate-400">{a}</span><b className="text-white">{b}</b></div><div className="mt-1 h-1.5 overflow-hidden rounded bg-white/[.06]"><div className="h-full bg-emerald-300" style={{width:`${v}%`}}/></div></div>)}</div></AirportPanel>
      </div>
    </div>
  </>;
}
    ...
    <div className="grid gap-4 xl:grid-cols-[1.4fr_.6fr]">
      ...
      <AirportPanel title={selected.name} subtitle={`${selected.id} · ${selected.zone}`}>
        <div className="space-y-4 p-4">
          <div className="flex items-center justify-between"><AirportStatusBadge status={selected.state === "Available" ? "optimized" : selected.state === "Under construction" ? "warning" : "normal"} label={selected.state}/><span className="text-[9px] text-slate-500">Readiness <b className="text-cyan-200">{selected.readiness}%</b></span></div>
          <div className="grid grid-cols-2 gap-2">{[["Area",selected.area],["Investor",selected.investor],["Build density",selected.density],["Permitted height",selected.floors]].map(([a,b])=><div key={a} className="rounded-lg border border-white/[.06] bg-white/[.025] p-3"><p className="text-[8px] text-slate-500">{a}</p><p className="mt-1 text-[10px] font-semibold text-white">{b}</p></div>)}</div>
          <div className="rounded-xl border border-cyan-400/12 bg-cyan-400/[.035] p-3"><p className="text-[8px] uppercase tracking-[.14em] text-cyan-300">Infrastructure connectivity</p><p className="mt-2 text-[10px] leading-relaxed text-slate-300">{selected.utilities}</p></div>
          <div className="space-y-2">{[["Planning dossier","Published"],["Investment workflow",selected.state === "Available" ? "Open campaign" : "Active"],["BIM / survey context","Linked"],["Environmental constraints","No critical conflict"]].map(([a,b],index)=><div key={a} className="flex items-center justify-between rounded-lg bg-white/[.025] px-3 py-2.5"><span className="text-[9px] text-slate-400">{a}</span><AirportStatusBadge status={index===1&&selected.state==="Under construction"?"warning":"normal"} label={b}/></div>)}</div>
...
function InfrastructureNetwork() {
  ...
  return <>
    ...
    <div className="grid gap-4 xl:grid-cols-[1.3fr_.7fr]">
      <AirportPanel title="Integrated infrastructure network topology"><div className="relative min-h-[500px] overflow-hidden p-4"><div className="absolute inset-0 bg-[#04111f]"/><div className="absolute inset-0 opacity-20" style={{backgroundImage:"linear-gradient(rgba(255,255,255,.1) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,.1) 1px,transparent 1px)",backgroundSize:"36px 36px"}}/><svg className="absolute inset-0 h-full w-full" viewBox="0 0 1000 500" preserveAspectRatio="none">{networks.map(([, , ,color],index)=><path key={color} d={`M 40 ${70+index*66} C 210 ${30+index*74}, 360 ${130+index*35}, 520 ${85+index*58} S 780 ${150+index*40}, 960 ${70+index*61}`} fill="none" stroke={color} strokeWidth={index===0?7:4} opacity=".68" strokeDasharray={index===2?"12 8":"0"}/>)}</svg><div className="relative grid grid-cols-3 gap-3">{networks.map(([name,length,status,color,Icon])=><button key={name} onClick={()=>toast.info(`${name} network context opened`)} className="rounded-xl border border-white/10 bg-[#06111f]/75 p-3 text-left backdrop-blur"><div className="flex items-center justify-between"><Icon size={16} style={{color}}/><AirportStatusBadge status={name==="Stormwater"?"warning":"normal"}/></div><p className="mt-3 text-[10px] font-semibold text-white">{name}</p><p className="mt-1 text-[9px] text-slate-500">{length} · {status}</p></button>)}</div><div className="absolute bottom-4 left-4 right-4 rounded-xl border border-cyan-400/15 bg-[#06111f]/88 p-4 backdrop-blur"><div className="grid grid-cols-5 gap-3">{[["Common ID","INF-000184"],["GIS feature","LineString ZM"],["BIM link","Utility corridor U-04"],["Sensors","28 linked"],["Work orders","2 open"]].map(([a,b])=><div key={a}><p className="text-[8px] text-slate-500">{a}</p><p className="mt-1 text-[9px] font-semibold text-white">{b}</p></div>)}</div></div></div></AirportPanel>
      <AirportPanel title="Underground-utility governance"><div className="space-y-2 p-3">{["Common utility corridor model","Depth and protection corridor checks","Permit-to-dig workflow","As-built and survey evidence","Isolation and outage impact tracing","Inspection and maintenance history"].map((item,index)=><div key={item} className="flex items-center gap-3 rounded-lg border border-white/[.06] bg-white/[.025] p-3"><span className="grid h-8 w-8 place-items-center rounded-lg bg-cyan-400/10 text-cyan-300">{index<3?<Network size={15}/>:<ClipboardCheck size={15}/>}</span><div className="flex-1"><p className="text-[10px] text-slate-300">{item}</p><p className="mt-1 text-[8px] text-slate-600">{index<3?"Planning and risk control":"Lifecycle evidence"}</p></div><CheckCircle2 size={14} className="text-emerald-300"/></div>)}</div></AirportPanel>
    </div>
  </>;
}
...
function BimFederation() {
  ...
  return <>
    ...
    <div className="grid gap-4 xl:grid-cols-[.55fr_1.45fr]">
      <AirportPanel title="Federated model portfolio"><div className="space-y-2 p-3">{models.map(([name,lod,mapped,state])=><button key={name} onClick={()=>setSelected(name)} className={`w-full rounded-xl border p-3 text-left ${selected===name?"border-cyan-400/25 bg-cyan-400/[.07]":"border-white/[.06] bg-white/[.025]"}`}><div className="flex items-center justify-between"><p className="text-[10px] font-semibold text-white">{name}</p><AirportStatusBadge status={state==="In progress"?"warning":"normal"} label={state}/></div><div className="mt-2 flex gap-3 text-[8px] text-slate-500"><span>{lod}</span><span>•</span><span>{mapped}</span></div></button>)}</div></AirportPanel>
      <div className="space-y-4">
        <AirportPanel title={`${selected} · Digital thread`} subtitle="GIS feature → BIM object → asset → sensor → alarm → work order → document">
          <div className="p-4"><div className="grid grid-cols-7 gap-2">{[["GIS","Spatial feature",Map],["BIM","IFC object",Boxes],["Asset","Master record",Building2],["IoT","Live telemetry",Activity],["Alarm","Operational event",ShieldCheck],["Work","CMMS workflow",Workflow],["Docs","Controlled evidence",FileBox]].map(([title,detail,Icon],index)=><div key={title as string} className="relative rounded-xl border border-cyan-400/12 bg-cyan-400/[.035] p-3 text-center"><span className="mx-auto grid h-9 w-9 place-items-center rounded-xl bg-cyan-400/10 text-cyan-300"><Icon size={16}/></span><p className="mt-2 text-[9px] font-semibold text-white">{title}</p><p className="mt-1 text-[8px] text-slate-600">{detail}</p>{index<6&&<ChevronRight size={14} className="absolute -right-[12px] top-1/2 z-10 -translate-y-1/2 text-cyan-300/45"/>}</div>)}</div></div>
        </AirportPanel>
        <div className="grid gap-4 xl:grid-cols-2">
          <AirportPanel title="BIM-GIS readiness"><div className="grid grid-cols-2 gap-2 p-4">{[["IFC properties","98.2%"],["Spatial placement","100%"],["Asset mapping","94.7%"],["Sensor mapping","91.8%"],["Document linkage","96.4%"],["Change history","100%"]].map(([a,b])=><div key={a} className="rounded-lg border border-white/[.06] bg-white/[.025] p-3"><p className="text-[8px] text-slate-500">{a}</p><p className="mt-1 text-sm font-semibold text-white">{b}</p></div>)}</div></AirportPanel>
          <AirportPanel title="Open coordination issues"><div className="space-y-2 p-3">{["Cable trench conflicts with drainage culvert","Four equipment objects missing asset IDs","As-built elevation differs from survey by 86 mm","Maintenance zone clearance requires review"].map((item,index)=><div key={item} className="rounded-lg border border-amber-400/12 bg-amber-400/[.035] p-3"><div className="flex justify-between gap-3"><p className="text-[9px] text-slate-300">{item}</p><AirportStatusBadge status={index===0?"critical":"warning"}/></div></div>)}</div></AirportPanel>
        </div>
      </div>
    </div>
  </>;
...
function SurveyMissions() {
  ...
  return <>
    ...
    <div className="grid gap-4 xl:grid-cols-[1.25fr_.75fr]">
      <AirportPanel title="Survey mission control"><div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left"><thead className="bg-white/[.025] text-[8px] uppercase tracking-[.12em] text-slate-500"><tr>{["Mission","Area","Purpose","Coverage","Status"].map(x=><th key={x} className="px-4 py-3">{x}</th>)}</tr></thead><tbody>{missions.map(([id,area,purpose,coverage,status])=><tr key={id} className="border-t border-white/[.055] text-[10px]"><td className="px-4 py-3 font-mono text-cyan-300">{id}</td><td className="px-4 py-3 text-white">{area}</td><td className="px-4 py-3 text-slate-400">{purpose}</td><td className="px-4 py-3 text-slate-300">{coverage}</td><td className="px-4 py-3"><AirportStatusBadge status={status==="Processing"||status==="Capturing"?"warning":"normal"} label={status}/></td></tr>)}</tbody></table></div></AirportPanel>
      <AirportPanel title="Mission data lifecycle"><div className="space-y-3 p-4">{[[UploadCloud,"Upload raw data","Checksum and mission metadata"],[CloudCog,"Process","Orthophoto, point cloud, DSM/DTM"],[ClipboardCheck,"Quality control","Accuracy and completeness"],[Database,"Publish","SLPK, web scene and catalog"],[Sparkles,"Detect change","Create issues and update digital twin"]].map(([Icon,title,detail],index)=><div key={title as string} className="flex gap-3 rounded-lg border border-white/[.06] bg-white/[.025] p-3"><span className="grid h-9 w-9 flex-none place-items-center rounded-xl bg-cyan-400/10 text-cyan-300"><Icon size={16}/></span><div><p className="text-[10px] font-semibold text-white">{title as string}</p><p className="mt-1 text-[8px] text-slate-500">{detail as string}</p></div>{index<3?<AirportStatusBadge status="normal" label="Ready"/>:<AirportStatusBadge status="info" label="Automated"/>}</div>)}</div></AirportPanel>
    </div>
  </>;
}
...
function ChangeDetection() {
  ...
  return <>
    ...
    <div className="grid gap-4 xl:grid-cols-[1.2fr_.8fr]">
      <AirportPanel title="Survey version comparison"><div className="p-4"><div className="relative h-[430px] overflow-hidden rounded-2xl border border-white/10 bg-[#06111f]"><div className="absolute inset-0 bg-[linear-gradient(125deg,rgba(34,211,238,.1),rgba(139,92,246,.08)),radial-gradient(circle_at_30%_40%,rgba(52,211,153,.18),transparent_28%)]"/><div className="absolute left-1/2 top-0 h-full w-px bg-white/35"/><div className="absolute left-4 top-4 rounded-lg border border-white/10 bg-[#06111f]/80 px-3 py-2"><p className="text-[8px] uppercase text-slate-500">Baseline</p><p className="mt-1 text-[10px] font-semibold text-white">Survey 2026-05-18</p></div><div className="absolute right-4 top-4 rounded-lg border border-cyan-400/20 bg-[#06111f]/80 px-3 py-2 text-right"><p className="text-[8px] uppercase text-cyan-300">Current</p><p className="mt-1 text-[10px] font-semibold text-white">Survey 2026-07-11</p></div>{[[28,34],[63,48],[45,70],[78,25]].map(([x,y],i)=><div key={i} style={{left:`${x}%`,top:`${y}%`}} className="absolute h-16 w-24 -translate-x-1/2 -translate-y-1/2 rounded border-2 border-amber-300/70 bg-amber-400/10 shadow-[0_0_20px_rgba(251,191,36,.18)]"><span className="absolute -top-5 left-0 text-[8px] font-semibold text-amber-200">Δ {i+1}</span></div>)}</div><div className="mt-4"><div className="flex justify-between text-[9px]"><span className="text-slate-400">Detection threshold</span><b className="text-cyan-200">{threshold} cm / {threshold*2} m²</b></div><input type="range" min="5" max="30" value={threshold} onChange={(e)=>setThreshold(Number(e.target.value))} className="mt-2 w-full accent-cyan-300"/></div></div></AirportPanel>
      <AirportPanel title="Detected changes and workflow"><div className="space-y-2 p-3">{changes.map(([type,area,value,state],index)=><button key={type as string} onClick={()=>toast.info(`${type} · ${area}`)} className="w-full rounded-xl border border-white/[.06] bg-white/[.025] p-3 text-left"><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-semibold text-white">{type}</p><p className="mt-1 text-[9px] text-slate-500">{area} · {value}</p></div><AirportStatusBadge status={state==="Priority"?"critical":state==="Review"?"warning":"normal"} label={state}/></div><div className="mt-3 flex items-center gap-2 text-[8px] text-slate-600"><span>Detect</span><ChevronRight size={10}/><span>Verify</span><ChevronRight size={10}/><span>Issue</span><ChevronRight size={10}/><span>Publish</span></div></button>)}</div></AirportPanel>
    </div>
  </>;
}

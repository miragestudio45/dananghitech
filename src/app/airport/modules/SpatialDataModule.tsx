        ...
        <div className="relative min-h-[470px] flex-1 overflow-hidden p-3 xl:min-h-0">
          ...
          <div className="pointer-events-none absolute inset-3 z-10 grid grid-cols-[180px_1fr] gap-4">
            ...
            <div className="relative">
              <div className="pointer-events-auto absolute right-11 top-0 rounded-lg border border-cyan-400/20 bg-[#06111f]/88 px-3 py-2 shadow-xl backdrop-blur"><p className="text-[10px] uppercase tracking-[.15em] text-cyan-300">Active layer</p><p className="mt-1 text-xs font-semibold text-white">{activeLayer}</p></div>
              {contextOpen ? <div className="pointer-events-auto absolute bottom-0 right-0 w-72 rounded-xl border border-cyan-400/20 bg-[#06111f]/94 p-3 shadow-2xl backdrop-blur-xl"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-[10px] uppercase tracking-[.14em] text-cyan-300">Selected spatial context</p><h3 className="mt-1 text-xs font-semibold leading-snug text-white">{selectedSite}</h3></div><button onClick={() => setContextOpen(false)} className="airport-icon-button h-7 w-7 flex-none" title="Đóng chi tiết"><Minimize2 size={12} /></button></div><div className="mt-3 grid grid-cols-2 gap-1.5">{[["GIS features","184"],["BIM objects","2,842"],["Linked assets","1,286"],["Open issues","4"]].map(([a,b]) => <div key={a} className="rounded-lg bg-white/[.035] px-2 py-1.5"><p className="text-[10px] text-slate-500">{a}</p><p className="mt-0.5 text-[13px] font-semibold text-white">{b}</p></div>)}</div></div> : <button onClick={() => setContextOpen(true)} className="airport-button pointer-events-auto absolute bottom-0 right-0 bg-[#06111f]/90 shadow-xl"><PanelRightOpen size={13} /> Chi tiết vị trí</button>}
            </div>
          </div>
        </div>
      </AirportPanel>
...
function TerrainElevation() {
  ...
  return <>
    ...
    <div className="grid gap-4 xl:grid-cols-[1.25fr_.75fr]">
      <AirportPanel title="Terrain and elevation analytical surface" action={<div className="flex rounded-lg border border-white/10 bg-white/[.03] p-1">{(["DTM","DSM"] as const).map((item) => <button key={item} onClick={() => setSurface(item)} className={`rounded px-3 py-1.5 text-[11px] font-bold ${surface === item ? "bg-cyan-300 text-slate-950" : "text-slate-500"}`}>{item}</button>)}</div>}>
        <div className="p-4">
          <div className="relative h-[360px] overflow-hidden rounded-2xl border border-cyan-400/12 bg-gradient-to-b from-[#0a2840] to-[#04111f]">
            ...
            <div className="absolute left-4 top-4 rounded-lg border border-white/10 bg-[#06111f]/80 px-3 py-2"><p className="text-[10px] uppercase tracking-[.14em] text-cyan-300">Active surface</p><p className="mt-1 text-sm font-semibold text-white">{surface === "DTM" ? "Bare-earth terrain" : "Buildings and vegetation surface"}</p></div>
            <div className="absolute bottom-4 left-4 right-4 grid grid-cols-4 gap-2">{[["Lowest","4.2 m"],["Median","34.8 m"],["Highest","128 m"],["Flood threshold","18.0 m"]].map(([a,b]) => <div key={a} className="rounded-lg border border-white/10 bg-[#06111f]/75 p-2 backdrop-blur"><p className="text-[10px] text-slate-500">{a}</p><p className="mt-1 text-xs font-semibold text-white">{b}</p></div>)}</div>
          </div>
        </div>
      </AirportPanel>

...
const SECTION_LABELS: Record<string, { vi: string; en: string }> = {
  ...
  "tenant-billing": { vi: "Mở Tenant Billing", en: "Open Tenant Billing" },
...
function buildReply(input: string, language: "vi" | "en"): AssistantReply {
  ...
  if (/(hóa đơn|billing|invoice|tenant|tiền)/i.test(normalized)) {
    return { sectionId: "tenant-billing", text: language === "vi" ? "Có 184/186 tenant đã sẵn sàng lập hóa đơn. Hai hồ sơ TEN-118 và TEN-042 đang chờ kiểm tra dữ liệu đo. Bạn có thể mở Tenant Billing để xem statement, tariff và luồng phê duyệt." : "184 of 186 tenants are billing-ready. TEN-118 and TEN-042 still require meter-data review. Open Tenant Billing to inspect statements, tariffs and the approval flow." };
  }
  ...
  return { text: language === "vi" ? "Mình có thể hỗ trợ phân tích điện, nước, BTU/gas, hóa đơn tenant, cảnh báo ngân sách và tình trạng gateway. Hãy chọn một gợi ý bên dưới hoặc mô tả bài toán năng lượng bạn muốn kiểm tra." : "I can help with electricity, water, BTU/gas, tenant billing, budget alerts and gateway status. Choose a suggestion below or describe the energy question you want to explore." };
}
...
export function YooEnergyAssistant({ language, onNavigateSection }: { language: "vi" | "en"; onNavigateSection: (sectionId: string) => void }) {
  ...
  return <>
    <AnimatePresence>
      {open && <motion.aside data-yooai-panel initial={{ opacity: 0, y: 18, scale: .97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 14, scale: .97 }} className="fixed bottom-[82px] right-3 z-[130] flex h-[min(620px,calc(100vh-100px))] w-[min(390px,calc(100vw-24px))] flex-col overflow-hidden rounded-2xl border border-cyan-300/20 bg-[var(--airport-sidebar)]/98 shadow-[0_24px_80px_var(--airport-shadow-heavy),0_0_38px_rgba(34,211,238,.1)] backdrop-blur-2xl sm:right-4">
        <div className="flex items-center gap-3 border-b border-white/[.07] bg-gradient-to-r from-cyan-400/[.09] to-violet-400/[.055] p-3">
          <motion.div animate={{ y: [0, -2, 0] }} transition={{ duration: 2.2, repeat: Infinity }} className="relative grid h-10 w-10 place-items-center rounded-xl border border-cyan-300/25 bg-cyan-300/10 text-cyan-200"><Bot size={21}/><span className="absolute -right-1 -top-1 h-2.5 w-2.5 rounded-full border-2 border-[var(--airport-sidebar)] bg-emerald-400"/></motion.div>
          <div className="min-w-0 flex-1"><div className="flex items-center gap-1.5"><h3 className="text-[12px] font-bold text-white">YooAI</h3><Sparkles size={12} className="text-violet-300"/></div><p className="mt-0.5 text-[10px] uppercase tracking-[.13em] text-slate-500">Energy intelligence copilot</p></div>
        ...
        <div className="airport-scrollbar min-h-0 flex-1 space-y-3 overflow-y-auto p-3">
          {messages.map((message)=><div key={message.id} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}><div className={`max-w-[88%] rounded-xl border px-3 py-2 text-[12px] leading-relaxed ${message.role === "user" ? "border-cyan-300/20 bg-cyan-300/10 text-cyan-50" : "border-white/[.07] bg-white/[.035] text-slate-300"}`}><p>{message.text}</p>{message.sectionId && <button type="button" onClick={()=>{onNavigateSection(message.sectionId!);setOpen(false)}} className="mt-2 flex items-center gap-1 text-[11px] font-semibold text-cyan-300 hover:text-cyan-100">{SECTION_LABELS[message.sectionId]?.[language] ?? message.sectionId}<ChevronRight size={12}/></button>}</div></div>)}
          {thinking && <div className="flex justify-start"><div className="rounded-xl border border-white/[.07] bg-white/[.035] px-3 py-2 text-[12px] text-slate-500"><span className="inline-flex gap-1"><i className="h-1.5 w-1.5 animate-pulse rounded-full bg-cyan-300"/><i className="h-1.5 w-1.5 animate-pulse rounded-full bg-cyan-300 [animation-delay:120ms]"/><i className="h-1.5 w-1.5 animate-pulse rounded-full bg-cyan-300 [animation-delay:240ms]"/></span></div></div>}
        </div>
        <div className="border-t border-white/[.07] p-3">
          <div className="mb-2 grid grid-cols-2 gap-1.5">{quickActions.map(([label,Icon])=><button type="button" key={label} onClick={()=>send(label)} className="flex items-center gap-1.5 rounded-lg border border-white/[.06] bg-white/[.025] px-2 py-2 text-left text-[10px] text-slate-400 hover:border-cyan-300/20 hover:text-cyan-100"><Icon size={12}/>{label}</button>)}</div>
          <div className="flex items-end gap-2 rounded-xl border border-white/[.08] bg-white/[.035] p-2"><textarea aria-label={language === "vi" ? "Nhập câu hỏi cho YooAI" : "Ask YooAI"} rows={2} value={query} onChange={(event)=>setQuery(event.target.value)} onKeyDown={(event)=>{if(event.key==="Enter"&&!event.shiftKey){event.preventDefault();send()}}} placeholder={language === "vi" ? "Hỏi về điện, nước, BTU, gas, billing…" : "Ask about energy, water, BTU, gas, billing…"} className="min-h-9 flex-1 resize-none bg-transparent text-[12px] leading-relaxed text-white outline-none placeholder:text-slate-600"/><button aria-label={language === "vi" ? "Gửi" : "Send"} onClick={()=>send()} disabled={!query.trim() || thinking} className="grid h-8 w-8 flex-none place-items-center rounded-lg bg-cyan-300 text-[var(--airport-accent-ink)] disabled:opacity-30"><Send size={14}/></button></div>
          <p className="mt-2 text-center text-[10px] text-slate-700">{language === "vi" ? "YooAI sử dụng dữ liệu trình diễn và không gửi lệnh tới thiết bị." : "YooAI uses presentation data and sends no device commands."}</p>
        </div>
      </motion.aside>}
    </AnimatePresence>
    <motion.button data-yooai-launcher aria-label={open ? (language === "vi" ? "Đóng YooAI" : "Close YooAI") : (language === "vi" ? "Mở trợ lý YooAI" : "Open YooAI assistant")} onClick={()=>setOpen((value)=>!value)} whileHover={{ scale: 1.04 }} whileTap={{ scale: .96 }} className="fixed bottom-4 right-3 z-[131] flex items-center gap-2 rounded-2xl border border-cyan-300/25 bg-[var(--airport-panel)]/96 p-2 pr-3 text-cyan-100 shadow-[0_14px_48px_var(--airport-shadow-heavy),0_0_28px_rgba(34,211,238,.14)] backdrop-blur-xl sm:right-4"><motion.span animate={{ y: [0, -2, 0], rotate: [0, -2, 2, 0] }} transition={{ duration: 2.4, repeat: Infinity }} className="relative grid h-10 w-10 place-items-center rounded-xl bg-gradient-to-br from-cyan-300/20 to-violet-400/15"><Bot size={21}/><span className="absolute -right-0.5 -top-0.5 h-2.5 w-2.5 rounded-full border-2 border-[var(--airport-panel)] bg-emerald-400"/></motion.span><span className="text-left"><b className="block text-[12px]">YooAI</b><small className="block text-[10px] text-slate-500">Energy copilot</small></span></motion.button>
  </>;
}
